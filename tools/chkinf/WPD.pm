...
#-------------------------------------------------------------------------------------------------#
# Verify that WUDFCoInstaller.dll is listed as one of the co-installers                           #
...
sub CheckWDFCoInstaller {
    ...
    my (%RequiredRegDirectives) = ('COINSTALLERS32'            => ["HKR", "", "0x10000", "WUDFCOINSTALLER.DLL", 0],
                                   );
                    ...
                    foreach $regline (@RegSectionLines) {
                        ...
                        if (defined($RequiredRegDirectives{uc $regparts[2]})) {
                            ...
                            while ($valindex < scalar @regparts) {
                                ...
                                if ($actualvalue eq $RequiredRegDirectives{uc $regparts[2]}[3]) {
                                    $valuefound = 1;
                                    last;
                                }
                            ...
                            AddError($Current_INF_File->getIndex($RegSection, $regcount), 2016, $RequiredRegDirectives{uc $regparts[2]}[3]) 
                                        unless ($valuefound == 1);
                        }                            
                    ...
                    foreach $svcline (@svclines) {
                        ...
                        if (uc $Directive eq 'SERVICEBINARY') {
                            ...
                            if ($Value =~ /^\$WinDir\$\\system32\\drivers\\wudfrd.sys$/i) {
                                ...
                                AddWarning($Current_INF_File->getIndex($svcSection,$svccount), 3005);
                            }
                        }
...
sub CheckWDF {
    ...
    foreach $line (@lines) {
        ...
        if (uc $Directive eq 'UMDFSERVICE') {
            ...
            my %UmdfImpersonationLevelValues = ('ANONYMOUS',
                                                'IDENTIFICATION',
                                                'IMPERSONATION',
                                                'DELEGATION'
                                                );
