...
int sdv_cancelFptr_val;


/* Suppress C4055: Casting data pointers to function pointers.
   This is acceptable as the OS Model will not be executed: */
#pragma warning(disable:4055)
PDRIVER_CANCEL sdv_cancelFptr = (PDRIVER_CANCEL) &sdv_cancelFptr_val;
#pragma warning(default:4055)
...
/* Suppress C4055: Casting data pointers to function pointers.
   This is acceptable as the OS Model will not be executed: */
#pragma warning(disable:4055)
PKSERVICE_ROUTINE  sdv_isr_function = (PKSERVICE_ROUTINE) &sdv_isr_val;
#pragma warning(default:4055)
...
/* Suppress C4055: Casting data pointers to function pointers.
   This is acceptable as the OS Model will not be executed: */
#pragma warning(disable:4055)
PKDEFERRED_ROUTINE  sdv_dpc = (PKDEFERRED_ROUTINE) &sdv_dpc_val;
#pragma warning(default:4055)
...
int sdv_harnessDeviceExtension;
int sdv_harnessDeviceExtension_two;
...
int sdv_kdpc_val;
struct _KDPC * sdv_kdpc = (struct _KDPC *) &sdv_kdpc_val;
    ...
    )
{
    ...
    default:
        ...
        pirp->PendingReturned = 1;
        break;

    }

    SdvAssume(pirp->IoStatus.Status != STATUS_PENDING);

    if (status != STATUS_PENDING) {
        SdvAssume( status == pirp->IoStatus.Status) ;
    }

    if (sdv_compFset) {
        sdv_stub_completion_begin(pirp);
        compRetStatus = (*sdv_compFptr)(DeviceObject, pirp, lcontext);
        sdv_stub_completion_end(pirp);

        if(compRetStatus == STATUS_MORE_PROCESSING_REQUIRED)
            sdv_stub_MoreProcessingRequired(pirp);
    }
