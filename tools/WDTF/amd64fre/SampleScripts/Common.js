...
//     5. The DeviceDepot does not yet support events.
//----------------------------------------------------------------------------
function JScript_WDTF()
{
  // Hold a reference to the actual WDTF COM object
  this.COM_WDTF = new ActiveXObject("WDTF.WDTF");
  
  this.DeviceDepot = new JScript_DeviceDepot(this.COM_WDTF.DeviceDepot);
  this.SystemDepot = new JScript_SystemDepot(this.COM_WDTF.SystemDepot);
  
  this.SetTraceLevel = 
    function(Level) { this.COM_WDTF.SetTraceLevel(Level); };
}
function JScript_SystemDepot(COM_SystemDepot)
{
  // Hold a reference to the actual SystemDepot COM object
  this.COM_SystemDepot = COM_SystemDepot;
  
  this.ThisSystem = new JScript_Target(this.COM_SystemDepot.ThisSystem);
  
  this.SetTraceLevel = 
    function(Level) { this.COM_SystemDepot.SetTraceLevel(Level); };
}
function JScript_DeviceDepot(COM_DeviceDepot)
{
  // Hold a reference to the actual DeviceDepot COM object
  this.COM_DeviceDepot = COM_DeviceDepot;
  
  this.RootDevice = new JScript_Target(this.COM_DeviceDepot.RootDevice);
  
  this.Query = 
    function(SDEL) { return JScript_Targets(this.COM_DeviceDepot.Query(SDEL)); };
  
  this.SetTraceLevel = 
    function(Level) { this.COM_DeviceDepot.SetTraceLevel(Level); };
}
function JScript_Targets(COM_Targets)
{
  ...
  {
    res[idx] = new JScript_Target(COM_Targets.Item(idx));
  }
  return res;
}
function JScript_Target(COM_Target)
{
  // Hold a reference to the actual Target COM object
  this.COM_Target = COM_Target;
  
  // Have a place to attach arbitrary user values just like the real "Target" interface
  this.GetContext = 
  ...
  // Map the GetValue() method directly
  this.GetValue = 
    function(value)
    {
  ...
  // Map the Eval() method directly
  this.Eval = 
    function(SDEL) { return this.COM_Target.Eval(SDEL); };
    
  // Map the GetRelations() method directly
  this.GetRelations = 
    function(SDEL, matchSDEL) { return JScript_Targets(this.COM_Target.GetRelations(SDEL, matchSDEL)); };
  
  // Map the GetInterface() method directly
  this.GetInterface = 
    function(progID, args, monikerSuffix) { return this.COM_Target.GetInterface(progID, args, monikerSuffix); };
  
  // Map the GetInterface() method, returning null
...
function JScript_CommonScenarioModule(WDTF, Devices)
{
  try
  {
    ...
    for(var idx in this.Devices)
    {
      ...
      Device.SetContext("DeviceInterface", Device.GetInterface("Action\\DeviceManagement\\Device"));
...
function SleepCycler(WDTF)
{
  try
  {
    this.Console = WDTF.SystemDepot.ThisSystem.TryGetInterface("Action\\Console");
  ...
  //----------------------------------------------------------------------------
  // Perform a simple sleep cycle on the system using the provided Action\Console
  // interface.
  //
  // Remarks:
