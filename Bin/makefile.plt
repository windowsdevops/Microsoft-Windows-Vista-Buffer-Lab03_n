...
__LATEST_NT_TARGET_VERSION=0x600
...
!ifndef KMDF_VER_PATH # default to current KMDF version
KMDF_VER_PATH = 10
!endif
...
!ifndef UMDF_VER_PATH # default to current UMDF version
UMDF_VER_PATH = 10
!endif
...
#
# __blddate and buildname.txt are misplaced.
# See makefil0, makefile.def, makefile.plt, set_build_date.pl, builddirectories.pm, scorch.pl.
#
!if exist ($(OBJECT_ROOT)\__blddate__)
!include $(OBJECT_ROOT)\__blddate__
!endif
...
!if defined(OBJECT_ROOT) && defined(USE_OBJECT_ROOT) && defined(MAKEDIR_RELATIVE_TO_BASEDIR)
...
#
# First set it independent of USE_OBJECT_ROOT. Makefile.def changes it
# back to MAKEDIR if USE_OBJECT_ROOT is not set. This way you can set it
# in sources files, though it is usually set in project.mk, and usable by makefil0.
#
...
# This ensures no files are left in TEMP -- via scorch/nuke/build /c -- all of which
