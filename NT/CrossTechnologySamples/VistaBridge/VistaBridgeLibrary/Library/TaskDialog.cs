...
{
    /// <summary>
    /// Encapsulates a new-to-Vista Win32 TaskDialog window - a powerful successor to the MessageBox available
    ...
    public class TaskDialog : IDialogControlHost, IDisposable
    {
        ...
        // as a NativeTaskDialog generator for all static Shows()
        ...
        // TODO: Various ctor overloads (similar to Show overloads)
        ...
        // TODO: Add lots of additional ones, and have them all call the most complex static
        public static TaskDialogResult Show(string msg)
        {
            return ShowCoreStatic(msg, TaskDialogDefaults.MainInstruction, TaskDialogDefaults.Caption);
        }
        ...
        // TODO: Remove, this is a proof of concept implementation for navigation
        public void Navigate()
        {
            if (NativeDialogShowing)
            {
                nativeDialog.NavigateTest();
            }
        }
        ...
        {
            if (checkBoxChecked.HasValue && String.IsNullOrEmpty(checkBoxText))
                throw new InvalidOperationException("Checkbox text must be provided to enable the dialog checkbox");

            // Progress bar validation
            // Make sure we don't have both a progress bar and marquee, and that the progress bar vals are valid -
            // the Win32 API will valiantly try to rationalize bizarre min/max/value combinations, but we'll save
            // it the trouble by validating
            if (progressBar != null && marquee != null)
                throw new NotSupportedException("Can't display both a progress bar and a marquee in same dialog");
            if (progressBar != null)
            {
                if (!progressBar.HasValidValues)
                    throw new ArgumentOutOfRangeException("Progress bar must have a value between the minimum and maxium values");
            }

            // Validate Buttons collection
            // Make sure we don't have buttons AND command-links - the Win32 API treats them as different
        ...
        // Searches list of controls and returns the ID of the default control, or null
        // if no default was specified
        private int FindDefaultButtonId(List<TaskDialogButtonBase> controls)
        {
            int found = NativeMethods.NO_DEFAULT_BUTTON_SPECIFIED;
            foreach (TaskDialogButtonBase control in controls)
            {
                if (control.Default)
                {
                    // Check if we've found a default in this list already
                    if (found != NativeMethods.NO_DEFAULT_BUTTON_SPECIFIED)
        ...
        // Helper to map the standard button IDs returned by TaskDialogIndirect
        // to the standard button ID enum - note that we can't just cast, as the Win32
        // typedefs differ incoming and outgoing
        private TaskDialogStandardButton MapButtonIdToStandardButton(int id)
        {
            switch ((NativeMethods.TASKDIALOG_COMMON_BUTTON_RETURN_ID)id)
            {
                case NativeMethods.TASKDIALOG_COMMON_BUTTON_RETURN_ID.IDOK:
                    return TaskDialogStandardButton.Ok;
                case NativeMethods.TASKDIALOG_COMMON_BUTTON_RETURN_ID.IDCANCEL:
                    return TaskDialogStandardButton.Cancel;
                case NativeMethods.TASKDIALOG_COMMON_BUTTON_RETURN_ID.IDABORT:
                    // Included for completeness in API - we can't pass in an Abort standard button
                    return TaskDialogStandardButton.None;
                case NativeMethods.TASKDIALOG_COMMON_BUTTON_RETURN_ID.IDRETRY:
                    return TaskDialogStandardButton.Retry;
                case NativeMethods.TASKDIALOG_COMMON_BUTTON_RETURN_ID.IDIGNORE:
                    // Included for completeness in API - we can't pass in an Ignore standard button
                    return TaskDialogStandardButton.None;
                case NativeMethods.TASKDIALOG_COMMON_BUTTON_RETURN_ID.IDYES:
                    return TaskDialogStandardButton.Yes;
                case NativeMethods.TASKDIALOG_COMMON_BUTTON_RETURN_ID.IDNO:
                    return TaskDialogStandardButton.No;
                case NativeMethods.TASKDIALOG_COMMON_BUTTON_RETURN_ID.IDCLOSE:
        ...
        // NOTE: we are going to require names be unique across both buttons and radio buttons,
        ...
        // We're explicitly implementing this interface as the user will never need to know about it
        // or use it directly - it is only for the internal implementation of "pseudo controls" within 
        // the dialogs
        ...
        {
            // If we're showing, we should never get here - the changing notification would have thrown and the 
        ...
        // All Raise*() methods are called by the NativeTaskDialog when various pseudo-controls
        // are triggered.
        internal void RaiseButtonClickEvent(int id)
        {
            ...
            // If a custom button was found, raise the event - if not, it's a standard button, and
        ...
        // Dispose pattern - cleans up data and structs for a) any native dialog currently
        // showing, and b) anything else that the outer TaskDialog has
        ...
        protected void Dispose(bool disposing)
        {
            if (!disposed)
            {
                ...
                if (disposing)
                {
                    ...
                    if (nativeDialog.ShowState == NativeDialogShowState.Showing)
                        nativeDialog.NativeClose();
                }
