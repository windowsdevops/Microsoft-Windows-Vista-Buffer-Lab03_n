...
certmgr.exe -del -r CurrentUser -s TrustedPeople -c -n %CLIENT_NAME%
...
echo ************
makecert.exe -sr CurrentUser -ss My -a sha1 -n CN=%CLIENT_NAME% -sky exchange -pe -r

IF DEFINED EXPORT_CLIENT (
    echo ************
    echo exporting client cert to client.cer
    echo ************
    certmgr.exe -put -r CurrentUser -s TrustedPeople -c -n %CLIENT_NAME% client.cer
) ELSE (
    echo ************
    echo copying client cert to server's CurrentUser store
    echo ************
    certmgr.exe -add -r CurrentUser -s My -c -n %CLIENT_NAME% -s -r LocalMachine TrustedPeople
)
GOTO :EOF
...
IF DEFINED EXPORT_SERVICE (
    ...
    echo ************
    certmgr.exe -put -r CurrentUser -s My -c -n %SERVER_NAME% service.cer
) ELSE (
    echo ************
