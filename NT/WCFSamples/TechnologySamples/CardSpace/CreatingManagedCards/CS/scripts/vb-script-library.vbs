...
on error resume next
	...
	' is it in the  %programfiles%\servicemodelsampletools directory?
	...
	' is it in the  %ProgramW6432%\servicemodelsampletools directory?
	path = wso.Environment("Process")("ProgramW6432")& "\servicemodelsampletools\findprivatekey.exe"
	if fso.FileExists(path) then
		GetFindPrivateKeyPath = path
		exit function
	end if

	' is it in the  %ProgramFiles(x86)%\servicemodelsampletools directory?
	path = wso.Environment("Process")("ProgramFiles(x86)")& "\servicemodelsampletools\findprivatekey.exe"
	if fso.FileExists(path) then
		GetFindPrivateKeyPath = path
		exit function
	end if
	...
	' Guess:
	path = "c:\program files\servicemodelsampletools\findprivatekey.exe"
	if fso.FileExists(path) then
		GetFindPrivateKeyPath = path
		exit function
	end if
	...
	logerror "And run the one time setup for WCF samples" 
	finished
...
function RemoveFromPath( currentPath  , pathToRemove)
	dim elements 
	dim e
	dim result
	result = ""
	elements = Split( currentPath, ";"  )
	for each e in elements
		e = TrimEx(e)
		if StrComp( e , pathToRemove )  then
			if len( e ) > 0 then 
				result  = result + e + ";"
			end if
		end if
