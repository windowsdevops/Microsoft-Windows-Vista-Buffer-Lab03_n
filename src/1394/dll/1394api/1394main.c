        ...
        while (TRUE) {
            ...
            if (SetupDiEnumDeviceInterfaces(hDevInfo, 0, guid, i, &deviceInterfaceData)) {
                ...
                if (!bReturn) {
                    ...
                    StringCbCopy(pDeviceData->deviceList[i].DeviceName,
    ...
    )
{
    ...
    if (hDevice != INVALID_HANDLE_VALUE) {
        ...
        ulStrLen = strlen(pVirtualDevice->DeviceID) + 1;

        if ( pVirtualDevice->fulFlags & IEEE1394_REQUEST_FLAG_UNICODE ) {
            p1394ApiReq = LocalAlloc(LPTR, sizeof(IEEE1394_API_REQUEST)+ (ulStrLen * sizeof(WCHAR)));
            p1394ApiReqLen = sizeof(IEEE1394_API_REQUEST)+(ulStrLen * sizeof(WCHAR));
        } else {
        ...
        if ( pVirtualDevice->fulFlags & IEEE1394_REQUEST_FLAG_UNICODE ) {
            StringCchCopyNW((LPWSTR)&pDevPnpReq->DeviceId,
                ulStrLen,
                (LPWSTR)pVirtualDevice->DeviceID, ulStrLen);
            ...
            StringCchCopyNA(&pDevPnpReq->DeviceId,
                ulStrLen,
                pVirtualDevice->DeviceID, ulStrLen);
