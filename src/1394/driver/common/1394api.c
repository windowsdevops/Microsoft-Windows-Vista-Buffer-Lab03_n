    ...
    )
{
    ...
    RtlZeroMemory (&LocalHostInfo5, sizeof (GET_LOCAL_HOST_INFO5));
    RtlZeroMemory (&LocalHostInfo6, sizeof (GET_LOCAL_HOST_INFO6));
	
    ENTER("t1394_GetLocalHostInformation");

    TRACE(TL_TRACE, ("nLevel = 0x%x\n", nLevel));
    TRACE(TL_TRACE, ("Information = 0x%p\n", Information));
    ...
    )
{
    ...
    TRACE(TL_TRACE, ("DeviceObject = 0x%p\n", DeviceObject));
    ...
    for (i=0; i<ulNumberOfDestinations; i++) {
        ...
        TRACE(TL_TRACE, ("hDestinationDeviceObjects[%d] = 0x%p\n", i, hDestinationDeviceObjects[i]));
    }
...
*/
    ...
    if (NT_SUCCESS(ntStatus)) {
        ...
        TRACE(TL_TRACE, ("UnitDependentDirectory = 0x%p\n",
    ...
    )
{
    ...
    if (nLevel == SET_LOCAL_HOST_PROPERTIES_GAP_COUNT) {
        ...
        TRACE(TL_TRACE, ("R0_SetLocalHostProps3 = 0x%p\n", R0_SetLocalHostProps3));
        ...
        if (R0_SetLocalHostProps3->fulFlags == SLHP_FLAG_ADD_CROM_DATA) {
            ...
            TRACE(TL_TRACE, ("CromData->Buffer = 0x%p\n", CromData->Buffer));
    ...
    )
{
    ...
    while (!IsListEmpty(&deviceExtension->BusResetIrps)) 
    {
        ...
        TRACE(TL_TRACE, ("BusResetIrp = 0x%p\n", BusResetIrp));

        // make this irp non-cancelable...
        prevCancel = IoSetCancelRoutine(BusResetIrp->Irp, NULL);

        TRACE(TL_TRACE, ("Completing BusResetIrp->Irp = 0x%p\n", BusResetIrp->Irp));

        KeReleaseSpinLock(&deviceExtension->ResetSpinLock, Irql);  //hyt temporary modify

        // and complete it...
        BusResetIrp->Irp->IoStatus.Status = STATUS_SUCCESS;
        IoCompleteRequest(BusResetIrp->Irp, IO_NO_INCREMENT);

        KeAcquireSpinLock(&deviceExtension->ResetSpinLock, &Irql); //hyt temporary modify
