    ...
    )
{
    ...
    if (NT_SUCCESS(ntStatus)) {
        ...
        *pSpeedSelected = pIrb->u.IsochAllocateBandwidth.SpeedSelected;
        ...
        TRACE(TL_TRACE, ("hBandwidth = 0x%p\n", *phBandwidth));
    ...
    )
{
    ...
    TRACE(TL_TRACE, ("R3_IsochDescriptor = 0x%p\n", R3_IsochDescriptor));
    ...
    for (i=0;i < nNumberOfDescriptors; i++) {
        ...
        TRACE(TL_TRACE, ("pR3TempDescriptor = 0x%p\n", pR3TempDescriptor));
        ...
        TRACE(TL_TRACE, ("pR3TempDescriptor->Data = 0x%p\n", &pR3TempDescriptor->Data));

        TRACE(TL_TRACE, ("pIsochDescriptor[%x] = 0x%p\n", i, &pIsochDescriptor[i]));
        ...
        if (pR3TempDescriptor->bUseCallback) {
            ...
            if (i != nNumberOfDescriptors-1) {
                ...
                TRACE(TL_TRACE, ("IsochAttachBuffers: pIsochDetachData = 0x%p\n", pIsochDetachData));
                TRACE(TL_TRACE, ("IsochAttachBuffers: pIsochDetachData->Irp = 0x%p\n", pIsochDetachData->Irp));
            }
        }
        else {
    ...
    )
{
    ...
    TRACE(TL_TRACE, ("hResource = 0x%p\n", hResource));
    TRACE(TL_TRACE, ("nNumberOfDescriptors = 0x%x\n", nNumberOfDescriptors));
    TRACE(TL_TRACE, ("IsochDescriptor = 0x%p\n", IsochDescriptor));
    ...
    )
{
    ...
    while (IsochResourceData) {
        ...
        TRACE(TL_TRACE, ("Removing hResource = 0x%p\n", hResource));
    ...
    )
{
    	...
    	(t1394_IsOnList(&IsochDetachData->IsochDetachList, &DeviceExtension->IsochDetachData)))
    {
        ...
        TRACE(TL_TRACE, ("IsochCallback: IsochDetachData = 0x%p\n", IsochDetachData));
        TRACE(TL_TRACE, ("IsochCallback: IsochDetachData->Irp = 0x%p\n", IsochDetachData->Irp));
        TRACE(TL_TRACE, ("IsochCallback: IsochDetachData->newIrp = 0x%p\n", IsochDetachData->newIrp));
    ...
    {
    	TRACE(TL_TRACE, ("IsochCallback: Entry 0x%p not on List 0x%p\n", 
    ...
    if (DeviceExtension) 
    {
		...
		if (t1394_IsOnList(&IsochDetachData->IsochDetachList, &DeviceExtension->IsochDetachData))
		{
        	...
        	if(KeCancelTimer(&IsochDetachData->Timer))
        	{
        	    ...
        	    TRACE(TL_TRACE, ("IsochTimeout: IsochDetachData = 0x%p\n", IsochDetachData));
	            TRACE(TL_TRACE, ("IsochTimeout: IsochDetachData->Irp = 0x%p\n", IsochDetachData->Irp));
	            TRACE(TL_TRACE, ("IsochTimeout: IsochDetachData->newIrp = 0x%p\n", IsochDetachData->newIrp));
...
{
    PISOCH_DETACH_DATA   IsochDetachData = (PISOCH_DETACH_DATA)Context;
    NTSTATUS        ntStatus = STATUS_SUCCESS;
    ULONG           i;
    ...
    )
{
   ...
   PISOCH_DETACH_DATA   IsochDetachData = (PISOCH_DETACH_DATA)Context;
	...
	if (!IsochDetachData)
	{
	    TRACE(TL_WARNING, ("IsochAttachCompletionRoutine: IsochDetachData = 0x%p\n", IsochDetachData));
        IoFreeIrp (Irp);
    ...
    if (!NT_SUCCESS(Irp->IoStatus.Status))
    {
        ...
        TRACE(TL_TRACE, ("IsochAttachCompletionRoutine: IsochDetachData = 0x%p\n", IsochDetachData));
        TRACE(TL_TRACE, ("IsochAttachCompletionRoutine: IsochDetachData->Irp = 0x%p\n", IsochDetachData->Irp));
        TRACE(TL_TRACE, ("IsochAttachCompletionRoutine: IsochDetachData->newIrp = 0x%p\n", IsochDetachData->newIrp));
