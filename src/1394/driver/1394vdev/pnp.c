    ...
    )
{
    ...
    TRACE(TL_TRACE, ("DriverObject = 0x%p\n", DriverObject));
    TRACE(TL_TRACE, ("PhysicalDeviceObject = 0x%p\n", PhysicalDeviceObject));
    ...
    TRACE(TL_TRACE, ("deviceExtension = 0x%p\n", deviceExtension));
    ...
    TRACE(TL_TRACE, ("StackDeviceObject = 0x%p\n", deviceExtension->StackDeviceObject));
    ...
    TRACE(TL_TRACE, ("PortDeviceObject = 0x%p\n", deviceExtension->PortDeviceObject));
    ...
    )
{
    ...
    while (TRUE) {
        ...
        if (!IsListEmpty(&deviceExtension->IsochDetachData)) {
            ...
            TRACE(TL_TRACE, ("Surprise Removal: IsochDetachData = 0x%p\n", IsochDetachData));

            KeCancelTimer(&IsochDetachData->Timer);
            KeReleaseSpinLock(&deviceExtension->IsochSpinLock, Irql);

            TRACE(TL_TRACE, ("Surprise Removal: IsochDetachData->Irp = 0x%pn", IsochDetachData->Irp));
    ...
    while (TRUE) {
        ...
        if (!IsListEmpty(&deviceExtension->IsochResourceData)) {
            ...
            TRACE(TL_TRACE, ("Surprise Removal: IsochResourceData = 0x%p\n", IsochResourceData));
            ...
            if (IsochResourceData) {
                ...
                TRACE(TL_TRACE, ("Surprise Removal: Freeing hResource = 0x%p\n", IsochResourceData->hResource));
    ...
    while (!IsListEmpty(&deviceExtension->BusResetIrps)) {
        ...
        TRACE(TL_TRACE, ("Surprise Removal: BusResetIrp->Irp = 0x%p\n", BusResetIrp->Irp));
