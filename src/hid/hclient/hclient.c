...
ReportToString(
   PHID_DATA    pData,
   __inout_bcount(iBuffSize) PCHAR szBuff,
   UINT          iBuffSize
);
...
VOID
vCreateUsageString(
    __in  PUSAGE        pUsageList,
    __out_bcount(uiStringSize) CHAR szString[],
    __in UINT                uiStringSize
)
{
    HRESULT stringReturn;
...
vCreateUsageAndPageString(
    __in  PUSAGE_AND_PAGE        pUsageList,
    __out_bcount(uiStringSize) CHAR szString[],
    __in  UINT                   uiStringSize
)
{
...
vCreateUsageValueStringFromArray(
    __in_ecount(uiBufferSize)PCHAR       pBuffer,
    __in UINT        uiBufferSize,
    __in USHORT      BitSize,
    __in USHORT      UsageIndex,
    __in_ecount(uiStringSize) CHAR szString[],
    __in  UINT       uiStringSize
)
/*++
Routine Description:
...
--*/
{
    ...
    //
    // Make sure we don't go pass our buffer
    //
    if(iByteIndex + (UINT)4 > uiBufferSize){
        stringReturn = StringCbPrintf(szString, uiStringSize, "ERR");
        return;
    }
    ...
    stringReturn = StringCbPrintf(szString, uiStringSize, "Usage value: %lu", ulValue);
