...
typedef struct _ECDISPLAY_PARAMS 
{
    ...
    UCHAR                     ReportID;
    __field_ecount(ListLength+1) 
    PCHAR                     szListString;
    __field_ecount(ListLength2+1) 
    PCHAR                     szListString2;
    __field_ecount(ListLength+1) 
    PUSAGE                    UsageList;
    __field_ecount(ListLength2+1) 
...
ECDisp_MakeGUIDString(
    IN  GUID guid, 
    __out_bcount(uiBuffSize) CHAR szString[],
	IN  UINT uiBuffSize
);
...
ECDisp_ConvertUlongListToValueList(
    ...
    __deref_out_bcount(*ValueListSize) PCHAR   *ValueList,
...
)
{
    ...
    for (i = 0; i < 8; i++) 
    {
        stringReturn = StringCbPrintf(&(szCharString[i]), sizeof(szCharString) - i,
                       "%x", guid.Data4[i]);
    }
