...
TapeSymbolicNamePersistencePreference(
    IN PDRIVER_OBJECT DriverObject,
    __in_opt IN PWCHAR KeyName,
    IN TAPE_PERSISTENCE_OPERATION Operation,
    __in_opt IN PWCHAR ValueName,
...
TapeGetSubKeyByIndex(
    ...
    OUT PHANDLE SubKey,
    __in IN ULONG ReturnStringLenInBytes,
    __out_bcount_opt(ReturnStringLenInBytes) OUT PWCHAR ReturnString
    );
...
TapeCreateNewDeviceSubKey(
    ...
    IN ULONG NewSubKeyNameSize,
    __out_bcount_opt(NewSubKeyNameSize) OUT PWCHAR NewSubKeyName
    );
...
TapeCreateNonPersistentSymbolicName(
    ...
    IN ULONG NonPersistentSymbolicNameSize,
    __out_bcount_opt(NonPersistentSymbolicNameSize) OUT PWCHAR NonPersistentSymbolicName
    );
...
TapeAssignSymbolicLink(
    ...
    IN BOOLEAN LegacyNameFormat,
    IN ULONG SymbolicNamePrefixSize,
    __in_bcount(SymbolicNamePrefixSize) IN PWCHAR SymbolicNamePrefix,
...
TapeDeassignSymbolicLink(
    ...
    #pragma alloc_text(PAGE, TapeClassAllocateSrbBuffer)		
...

{
    ...
    if (TapeInitData->InitDataSize < sizeof(TAPE_INIT_DATA)) {

        return STATUS_INVALID_PARAMETER;

    } else if (TapeInitData->InitDataSize < sizeof(TAPE_INIT_DATA_EX)) {
...

{
    ...
    do {
        RtlStringCbCopyNA(deviceNameBuffer,
                          sizeof(deviceNameBuffer),
                          TAPE_DEVICE_PREFIX_A,
                          strlen(TAPE_DEVICE_PREFIX_A));

        RtlStringCchPrintfExA(deviceNameBuffer + strlen(deviceNameBuffer),
                              sizeof(deviceNameBuffer) - strlen(deviceNameBuffer),
                              NULL,
                              NULL,
...

{
    ...
    if (tapeData->DosNameCreated) {
        ...
        //
        TapeDeassignSymbolicLink(wcslen(TAPE_DOS_DEVICES_PREFIX_W) * sizeof(WCHAR),
    ...
    if (tapeData->PnpNameLinkCreated) {
        ...
        //
        TapeDeassignSymbolicLink(wcslen(TAPE_DEVICE_PREFIX_W) * sizeof(WCHAR),
...

{
    ...
    if(TapeReadWrite(DeviceObject, Irp) != STATUS_SUCCESS) {
...

{
    ...
    for (i = 0; i < irpCount; i++) {
        ...
        if(TapeReadWrite(Fdo, newIrp) != STATUS_SUCCESS) {
        ...
        srb = * (volatile PSCSI_REQUEST_BLOCK *) &newIrpStack->Parameters.Others.Argument1;
...

{
    ...
    if(!srb) {
        DebugPrint((1,
                  "TapeReadWrite: Can't allocate Srb\n"));
        return STATUS_INSUFFICIENT_RESOURCES;
    } 
...

{
    ...
    if (!PersistencePreference) {
        ...
        status = TapeAssignSymbolicLink(tapeData->PnpNameDeviceNumber,
                                        ...
                                        nameFormatIsLegacy,
                                        wcslen(TAPE_DOS_DEVICES_PREFIX_W) * sizeof(WCHAR),
                                        TAPE_DOS_DEVICES_PREFIX_W,
                                        &tapeData->SymbolicNameDeviceNumber);
        ...
        if (NT_SUCCESS(status)) {
            ...
            RtlStringCchPrintfExW(wideNameBuffer,
                                  sizeof(wideNameBuffer) / sizeof(wideNameBuffer[0]),
                                  NULL,
                                  NULL,
                                  STRSAFE_FILL_BEHIND_NULL | STRSAFE_FILL_ON_FAILURE,
            ...
            status = TapeAssignSymbolicLink(tapeData->PnpNameDeviceNumber,
                                            &unicodeRegistryDevice,
                                            nameFormatIsLegacy,
                                            wcslen(TAPE_DEVICE_PREFIX_W) * sizeof(WCHAR),
                                            TAPE_DEVICE_PREFIX_W,
                                            NULL);
...

{
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (ReturnStringLenInBytes > keyBasicInfo->NameLength) {
...
Arguments:
    ...
    NewSubKeyNameSize - size of the pre-allocated buffer in bytes
    ...
    for (index = *FirstNumeralToUse; index < MAXLONG && index != MAXLONG; index--) {
        ...
        if (!NT_SUCCESS(status)) {
            ...
            if (REG_OPENED_EXISTING_KEY == disposition) {
                ...
                //
                RtlStringCchCopyNW(NewSubKeyName,
                                   NewSubKeyNameSize / sizeof(WCHAR),
                                   wideNameBuffer,
                                   wcslen(wideNameBuffer));
...
Arguments:
    ...
    NonPersistentSymbolicNameSize - size of the pre-allocated buffer in bytes
...

{
    ...
    if (NULL == RootKey) {
        ...
        RtlStringCchPrintfExW(NonPersistentSymbolicName,
                              NonPersistentSymbolicNameSize / sizeof(WCHAR),
                              NULL,
                              NULL,
                              STRSAFE_FILL_BEHIND_NULL | STRSAFE_FILL_ON_FAILURE,
                              TAPE_NAME_FORMAT_W,
        ...
        for (index = *FirstNumeralToUse; index < MAXLONG && index != MAXLONG; index++) {
            ...
            if (NT_SUCCESS(status)) {
                ...
                //
                RtlStringCchCopyNW(NonPersistentSymbolicName,
                                   NonPersistentSymbolicNameSize / sizeof(WCHAR),
                                   wideNameBuffer,
                                   wcslen(wideNameBuffer));
...
Arguments:
    ...
    SymbolicNamePrefixSize - size of the SymbolicNamePrefix buffer in bytes
...

{
    ...
    RtlStringCchCopyNW(wideDeviceNameBuffer,
                       sizeof(wideDeviceNameBuffer) / sizeof(wideDeviceNameBuffer[0]),
                       TAPE_DEVICE_PREFIX_W,
                       wcslen(TAPE_DEVICE_PREFIX_W));

    RtlStringCchPrintfExW(wideDeviceNameBuffer + wcslen(wideDeviceNameBuffer),
                          sizeof(wideDeviceNameBuffer) / sizeof(wideDeviceNameBuffer[0]) - wcslen(wideDeviceNameBuffer),
                          NULL,
                          NULL,
                          STRSAFE_FILL_BEHIND_NULL | STRSAFE_FILL_ON_FAILURE,
    ...
    RtlStringCchCopyNW(wideSymbolicNameBuffer,
                       sizeof(wideSymbolicNameBuffer) / sizeof(wideSymbolicNameBuffer[0]),
                       SymbolicNamePrefix,
                       SymbolicNamePrefixSize / sizeof(WCHAR));

    RtlStringCchCatExW(wideSymbolicNameBuffer,
                       sizeof(wideSymbolicNameBuffer) / sizeof(wideSymbolicNameBuffer[0]),
                       L"\\",
                       NULL,
                       NULL,
...

{
    ...
    RtlStringCchPrintfExW(wideSymbolicNameBuffer + wcslen(wideSymbolicNameBuffer),
                          sizeof(wideSymbolicNameBuffer) / sizeof(wideSymbolicNameBuffer[0]) - wcslen(wideSymbolicNameBuffer),
                          NULL,
                          NULL,
                          STRSAFE_FILL_BEHIND_NULL | STRSAFE_FILL_ON_FAILURE,
...

{
    ...
    if ((DebugPrintLevel <= (TapeClassDebug & 0x0000ffff)) ||
        ...
        RtlStringCchVPrintfA(TapeClassBuffer,
                             TAPE_DEBUG_PRINT_BUFF_LEN,
                             DebugMessage,
                             ap);
