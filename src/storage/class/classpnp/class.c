...
#include "class.tmh"
    ...
    #pragma alloc_text(PAGE, ClassPageControlModeSense)
            ...
            if (transferByteCount == 0) {
                ...
                if (commonExtension->DriverExtension->InitData.ClassStartIo) {
                    ...
                    if (commonExtension->IsFdo){
                         ...
                         */
                        if (IsIdleRequestSupported(fdoData, Irp)) {
                        ...
                        IoCopyCurrentIrpStackLocationToNext(Irp);
                        ClassReleaseRemoveLock(DeviceObject, Irp);
                        status = IoCallDriverStackSafeDefault(lowerDeviceObject, Irp);
                    }
                }
            }
        }
...
VOID InterpretCapacityData(PDEVICE_OBJECT Fdo, PREAD_CAPACITY_DATA_EX ReadCapacityData)
{
    ...
    if (bytesPerSector == 0) {
        ...
        if (bytesPerSector & (bytesPerSector-1)){
            TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_INIT, "FDO %ph has non-standard sector size 0x%x.", Fdo, bytesPerSector));
            do {
    ...
    )
{
    ...
    if (TEST_FLAG(srb->SrbFlags, SRB_CLASS_FLAGS_FREE_MDL)) {
        CLEAR_FLAG(srb->SrbFlags, SRB_CLASS_FLAGS_FREE_MDL);
        IoFreeMdl(Irp->MdlAddress);
        Irp->MdlAddress = NULL;
    }
...
retry:
    ...
    status = IoCallDriverStackSafeDefault(fdoExtension->CommonExtension.LowerDeviceObject, irp);
    ...
    if (SRB_STATUS(Srb->SrbStatus) != SRB_STATUS_SUCCESS) {
        ...
        TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_GENERAL, "ClassSendSrbSynchronous - srb %ph failed (op=%s srbstat=%s(%xh), irpstat=%xh, sense=%s/%s/%s)", Srb, DBGGETSCSIOPSTR(Srb),
            ...
            *Status = STATUS_INVALID_DEVICE_REQUEST;
            ...
            case SCSI_ADSENSE_ILLEGAL_BLOCK: {
                ...
                *Status = STATUS_NONEXISTENT_SECTOR;
                ...
                if ((Fdo->DeviceType == FILE_DEVICE_DISK) && (IS_SCSIOP_READWRITE(cdb->CDB10.OperationCode))) {
    ...
    )
{
    ...
    if (senseBufferMdl){
        ...
        if (pkt){
            ...
            ASSERT(Length <= 0x0ff);
            KeInitializeEvent(&event, SynchronizationEvent, FALSE);
            SetupModeSenseTransferPacket(pkt, &event, ModeSenseBuffer, (UCHAR)Length, PageMode, &pseudoIrp, MODE_SENSE_CURRENT_VALUES);
            SubmitTransferPacket(pkt);
            KeWaitForSingleObject(&event, Executive, KernelMode, FALSE, NULL);
            ...
            if (NT_SUCCESS(pseudoIrp.IoStatus.Status)){
                 ...
                 *  so this is not necessarily a device/driver bug.
                 */
                TracePrint((TRACE_LEVEL_WARNING, TRACE_FLAG_GENERAL, "ClassModeSense on Fdo %ph failed with status %xh.", Fdo, pseudoIrp.IoStatus.Status));
            }
        }
...
/*++
ClassPageControlModeSense()

Routine Description:

    This routine sends a mode sense command to a target ID and returns
    when it is complete. This routine take the Page Control field as
    as an argument.
...
Arguments:
    ...
    PageControl - Supplies teh page control value of the request.
    ...
    )
{
    ...
    if (senseBufferMdl) {
        ...
        if (pkt) {
            ...
            ASSERT(Length <= 0x0ff);
            KeInitializeEvent(&event, SynchronizationEvent, FALSE);
            SetupModeSenseTransferPacket(pkt, &event, ModeSenseBuffer, (UCHAR)Length, PageMode, &pseudoIrp, PageControl);
            SubmitTransferPacket(pkt);
            KeWaitForSingleObject(&event, Executive, KernelMode, FALSE, NULL);
    ...
    )
{
    ...
    IoCallDriverStackSafeDefault(fdoExtension->CommonExtension.LowerDeviceObject, Irp);
    ...
    )
{
    ...
    if ((controlCode == IOCTL_SCSI_PASS_THROUGH) ||
        ...
        status = IoCallDriverStackSafeDefault(commonExtension->LowerDeviceObject, Irp);
        goto SetStatusAndReturn;
    }
    ...
    )
{
    ...
    return IoCallDriverStackSafeDefault(commonExtension->LowerDeviceObject, Irp);
    ...
    )
{
    ...
    TRY {
        ...
        status = RtlStringCchPrintfA(buffer,
                sizeof(buffer)-1,
        ...
        status = RtlStringCchPrintfA(buffer, sizeof(buffer)-1, "%s%d", DeviceName, DeviceNumber);
        if (!NT_SUCCESS(status)) {
            LEAVE;
        }
    ...
    )
{
    ...
    status = IoCallDriverStackSafeDefault(TargetDeviceObject, Irp);
    ...
    )
{
    ...
    IoCallDriverStackSafeDefault(lowerDevice, irp);
    ...
    )
{
    ...
    while (retryList != NULL) {
        ...
        IoCallDriverStackSafeDefault(commonExtension->LowerDeviceObject, irp);
...
ClassRetryRequest(
    ...
    IN PIRP           Irp,
    __in __range(0,MAXIMUM_RETRY_FOR_SINGLE_IO_IN_100NS_UNITS) // 100 seconds; already an assert on this...
...
Routine Description:
    ...
    This routine will send IOCTL_DISK_UPDATE_PROPERTIES to the disk driver
    ...
    )
{
    ...
    if (irp != NULL) {
        status = IoCallDriverStackSafeDefault(topOfStack, irp);
        if (status == STATUS_PENDING) {
            KeWaitForSingleObject(&event, Executive, KernelMode, FALSE, NULL);
            status = ioStatus.Status;
        }
