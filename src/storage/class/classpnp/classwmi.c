...
ConvertTickToDateTime(
    ...
    __out_ecount(TIME_STRING_LENGTH) PWCHAR String
    );
    ...
    switch(minorFunction)
    {
        case IRP_MN_REGINFO:
        {
            ...
            if (NT_SUCCESS(status))
            {
                ...
                RtlULongAdd(registryPathOffset, regPath->Length, &bufferNeeded);
                RtlULongAdd(bufferNeeded, sizeof(USHORT), &bufferNeeded);
                ...
                if (bufferNeeded <= bufferSize)
                {
                    ...
                    {
                        RtlULongAdd(nameOffset, sizeof(WCHAR), &bufferNeeded);
                        RtlULongAdd(bufferNeeded, name.Length, &bufferNeeded);
                        ...
                        if (bufferSize >= bufferNeeded){
                            ...
                            *stringPtr++ = name.Length;
                            ...
                            ASSERT(bufferSize >= bufferNeeded);
                            status = STATUS_INVALID_BUFFER_SIZE;
                        }
                    }

                    RtlULongAdd(mofResourceOffset, sizeof(WCHAR), &bufferNeeded);
                    RtlULongAdd(bufferNeeded, mofName.Length, &bufferNeeded);
                    ...
                    RtlULongAdd(registryPathOffset, sizeof(WCHAR), &bufferNeeded);
                    RtlULongAdd(bufferNeeded, regPath->Length, &bufferNeeded);
