...
NTSTATUS InitializeTransferPackets(PDEVICE_OBJECT Fdo)
{
    ...
    if (commonExt->DriverExtension->WorkingSet != NULL) {
        ...
        {
            MinWorkingSetTransferPackets = workingSet->XferPacketsWorkingSetMinimum;
            // adjust maximum upwards if needed
            if (MaxWorkingSetTransferPackets < MinWorkingSetTransferPackets)
            {
                MaxWorkingSetTransferPackets = MinWorkingSetTransferPackets;
            }
        }
        // override only if non-zero
        if (workingSet->XferPacketsWorkingSetMaximum != 0)
        {
            MaxWorkingSetTransferPackets = workingSet->XferPacketsWorkingSetMaximum;
            // adjust minimum downwards if needed
            if (MinWorkingSetTransferPackets > MaxWorkingSetTransferPackets)
            {
                MinWorkingSetTransferPackets = MaxWorkingSetTransferPackets;
            }
        }
...
VOID EnqueueFreeTransferPacket(PDEVICE_OBJECT Fdo, PTRANSFER_PACKET Pkt)
{
    ...
    if (fdoData->NumFreeTransferPackets >= fdoData->NumTotalTransferPackets){
        ...
        if (fdoData->NumTotalTransferPackets > MaxWorkingSetTransferPackets){
            ...
            TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_RW, "Exiting stress, block freeing (%d-%d) packets.", fdoData->NumTotalTransferPackets, MaxWorkingSetTransferPackets));
            ...
            while ((fdoData->NumFreeTransferPackets >= fdoData->NumTotalTransferPackets) &&
                ...
                if (pktToDelete){
                    ...
                    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_RW, "Extremely unlikely condition (non-fatal): %d packets dequeued at once for Fdo %p. NumTotalTransferPackets=%d (1).", MaxWorkingSetTransferPackets, Fdo, fdoData->NumTotalTransferPackets));
                    break;
                }
            }
            KeReleaseSpinLock(&fdoData->SpinLock, oldIrql);
        ...
        if (fdoData->NumTotalTransferPackets > MinWorkingSetTransferPackets){
            ...
            TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_RW, "Exiting stress, lazily freeing one of %d/%d packets.", fdoData->NumTotalTransferPackets, MinWorkingSetTransferPackets));
            ...
            if ((fdoData->NumFreeTransferPackets >= fdoData->NumTotalTransferPackets) &&
                ...
                if (pktToDelete){
                    ...
                    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_RW, "Extremely unlikely condition (non-fatal): %d packets dequeued at once for Fdo %p. NumTotalTransferPackets=%d (2).", MinWorkingSetTransferPackets, Fdo, fdoData->NumTotalTransferPackets));
                }
            }
            KeReleaseSpinLock(&fdoData->SpinLock, oldIrql);
...
NTSTATUS SubmitTransferPacket(PTRANSFER_PACKET Pkt)
{
    ...
    BOOLEAN idleRequest = IsIdleRequestSupported(fdoData, Pkt->OriginalIrp);
    ...
    return IoCallDriverStackSafeDefault(nextDevObj, Pkt->Irp);
}
