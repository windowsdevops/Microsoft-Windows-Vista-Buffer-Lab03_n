...
BOOLEAN
PrependSzToMultiSz(
    __in        LPTSTR  SzToPrepend,
    __deref_inout LPTSTR *MultiSz
    );

size_t
MultiSzLength(
    __in IN LPTSTR MultiSz
    );

size_t
MultiSzSearchAndDeleteCaseInsensitive(
    __in IN  LPTSTR  FindThis,
    __in IN  LPTSTR  FindWithin,
...
int __cdecl _tmain(int argc, __in_ecount(argc) _TCHAR ** argv)
{
