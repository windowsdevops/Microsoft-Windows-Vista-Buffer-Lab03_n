...
--*/
{
    PCSTR index1 = String1;
    PCSTR index2 = String2;
	char  char1;
	char  char2;
    ...
    while(*index1) {
		...
		//
		//	Convert characters to lower case before comparing
		//
        if (char1 >= 'A' && char1 <= 'Z') {
            char1 = char1 + ('a' - 'A');
        }
		...
		//
		//	Fail the comparison as soon as we hit a mismatch
		//
        if (char1 != char2) {
            return FALSE;
        }
...
--*/
{
    ...
    //
    if (I2OStrCompI("ntldr=1", ArgumentString)) {
    ...
    } else if (I2OStrCompI("dump=1", ArgumentString)) {
...
--*/
{
    ...
    if (!(deviceInfo->Characteristics & I2O_BSA_DEV_CAP_REMOVABLE_MEDIA)) {
		if (sizeof(READ_CAPACITY_DATA) > Srb->DataTransferLength)
		{
            DebugPrint((1, "I2ODriveCapacity: Data buffer too small for READ_CAPACITY data\n"));
...
--*/
{
    ...
    struct {
        I2O_UTIL_PARAMS_GET_MESSAGE msg;
        I2O_PARAM_SCALAR_OPERATION  scalar;
    } cmd;
    ...
    // Setup the message header.
    //
    cmd.msg.StdMessageFrame.VersionOffset    = I2O_VERSION_11 | I2O_PARAMS_SGL_VER_OFFSET;
    cmd.msg.StdMessageFrame.MsgFlags         = 0;
    cmd.msg.StdMessageFrame.TargetAddress    = deviceInfo->Lct.LocalTID;
    cmd.msg.StdMessageFrame.InitiatorAddress = I2O_HOST_TID;
    cmd.msg.StdMessageFrame.Function         = I2O_UTIL_PARAMS_GET;
    cmd.msg.StdMessageFrame.InitiatorContext = 0;
    ...
    cmd.msg.StdMessageFrame.InitiatorContext = pointer64ToU32.u.LowPart;
    cmd.msg.TransactionContext = pointer64ToU32.u.HighPart;

    cmd.msg.StdMessageFrame.MessageSize = (sizeof(I2O_UTIL_PARAMS_GET_MESSAGE) -
                                           sizeof(I2O_SG_ELEMENT) +
                                           sizeof(I2O_SGE_IMMEDIATE_DATA_ELEMENT) +
                                           sizeof(I2O_PARAM_SCALAR_OPERATION)) >> 2;

    cmd.msg.SGL.u.ImmediateData.FlagsCount.Count = sizeof(I2O_PARAM_SCALAR_OPERATION);
    cmd.msg.SGL.u.ImmediateData.FlagsCount.Flags = I2O_SGL_FLAGS_LAST_ELEMENT  |
                                                   I2O_SGL_FLAGS_END_OF_BUFFER |
                                                   I2O_SGL_FLAGS_IMMEDIATE_DATA_ELEMENT;
    ...
    //
    scalar = (PI2O_PARAM_SCALAR_OPERATION)((PUCHAR)&cmd.msg.SGL +
                                           sizeof(I2O_SGE_IMMEDIATE_DATA_ELEMENT));
    scalar->OpList.OperationCount   = 1;
    scalar->OpBlock.Operation       = I2O_PARAMS_OPERATION_FIELD_GET;
...
--*/
{
    ...
    if ((cdb->MODE_SENSE.PageCode == MODE_PAGE_CACHING) ||
        ...
        //
        //  Ensure that we have room in the data buffer for the header
        //
        if (sizeof(MODE_PARAMETER_HEADER) > requestLength)
        {
            DebugPrint((1,"I2OModeSense: Data buffer too small for mode sense header\n"));
            return SRB_STATUS_ERROR;
        }
