    ...
    )
{
    ...
    if (SRB_STATUS(srb.SrbStatus) == SRB_STATUS_SUCCESS ||
        ...
        if (RtlCompareMemory(changerData->InquiryData.VendorId,"HP      ",8) == 8) {
            ...
            } else if (RtlCompareMemory(changerData->InquiryData.ProductId,"DAT72X6",7) == 7) {
                changerData->DriveID = HP_DDS4;
            }
...

{
    ...
    if (readElementStatus->VolumeTagInfo) {
        ...
        if ((changerData->DriveID == SONY_TSL) ||
            ...
            status = RtlULongMult(sizeof(SONY_ELEMENT_DESCRIPTOR),
            ...
            status = RtlULongMult(sizeof(HP_ELEMENT_DESCRIPTOR),
                                  readElementStatus->ElementList.NumberOfElements,
                                  &elementStatusLength);
            if (!NT_SUCCESS(status)) {
                return status;
        ...
        status = RtlULongMult(sizeof(ELEMENT_STATUS_PAGE), statusPages, &statusPageLength);
        if (!NT_SUCCESS(status)) {
            return status;
        }

        status = RtlULongAdd(sizeof(ELEMENT_STATUS_HEADER), statusPageLength, &length);
        if (!NT_SUCCESS(status)) {
            return status;
        }

        status = RtlULongAdd(length, elementStatusLength, &length);
        if (!NT_SUCCESS(status)) {
            return status;
        }
    }
