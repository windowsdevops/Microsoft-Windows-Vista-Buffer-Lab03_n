...

{
    ...
    if (NT_SUCCESS(status)) {
        ...
        //
        status = RtlULongMult(sizeof(SONY_ELEMENT_DESCRIPTOR_PLUS), returnElements, &elementStatusLength);
        if (!NT_SUCCESS(status)) {
            return status;
        }

        status = RtlULongAdd(sizeof(ELEMENT_STATUS_HEADER), sizeof(ELEMENT_STATUS_PAGE), &requestLength);
        if (!NT_SUCCESS(status)) {
            return status;
        }

        status = RtlULongAdd(requestLength, elementStatusLength, &requestLength);
        if (!NT_SUCCESS(status)) {
            return status;
        }
        ...
        if ((status == STATUS_SUCCESS) || (status == STATUS_DATA_OVERRUN)) {
            ...
            if (numberElements) {
                ...
                //
                // Output buffer is of at least sizeof(READ_ELEMENT_ADDRESS_INFO).
                // If it were lesser than this, mcd itself would have failed the request
                // with STATUS_INFO_LENGTH_MISMATCH.
                // And even if number of elements is 0, we don't have to worry about
                // the data that will be returned in the array (consisting of 1 element
                // by default) as we first zero it out.
                //
                Irp->IoStatus.Information = sizeof(READ_ELEMENT_ADDRESS_INFO);
            }
        } else {
