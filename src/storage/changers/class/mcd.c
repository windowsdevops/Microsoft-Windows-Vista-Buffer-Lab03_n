...

{
    ...
    case IOCTL_CHANGER_GET_ELEMENT_STATUS:
        ...
        if (irpStack->Parameters.DeviceIoControl.InputBufferLength <
            ...
            //
            status = STATUS_SUCCESS;
            if ((MAXULONG / sizeof(CHANGER_ELEMENT_STATUS) < readElementStatus->ElementList.NumberOfElements) ||
                (MAXULONG / sizeof(CHANGER_ELEMENT_STATUS_EX) < readElementStatus->ElementList.NumberOfElements)) {

                status = STATUS_BUFFER_OVERFLOW;

            } else if ((outputBuffLen < lengthEx) && (outputBuffLen < length)) {
...

{
    ...
    if (mcdClassData->DosNameCreated) {
        ...
        //
        ChangerDeassignSymbolicLink(wcslen(MCD_DOS_DEVICES_PREFIX_W) * sizeof(WCHAR),
    ...
    if (mcdClassData->PnpNameLinkCreated) {
        ...
        //
        ChangerDeassignSymbolicLink(wcslen(MCD_DEVICE_PREFIX_W) * sizeof(WCHAR),
                                    MCD_DEVICE_PREFIX_W,
                                    mcdClassData->SymbolicNameDeviceNumber,
                                    !mcdClassData->PersistencePreferred);
...

{
    ...
    if (!PersistencePreference) {
        ...
        status = ChangerAssignSymbolicLink(mcdClassData->PnpNameDeviceNumber,
                                           ...
                                           nameFormatIsLegacy,
                                           wcslen(MCD_DOS_DEVICES_PREFIX_W) * sizeof(WCHAR),
                                           MCD_DOS_DEVICES_PREFIX_W,
                                           &mcdClassData->SymbolicNameDeviceNumber);
        ...
        if (NT_SUCCESS(status)) {
            ...
            status = ChangerAssignSymbolicLink(mcdClassData->PnpNameDeviceNumber,
                                               &unicodeRegistryDevice,
                                               nameFormatIsLegacy,
                                               wcslen(MCD_DEVICE_PREFIX_W) * sizeof(WCHAR),
                                               MCD_DEVICE_PREFIX_W,
                                               NULL);
...

{
    ...
    if (DebugPrintLevel <= MCDebug) {
        ...
        RtlStringCchVPrintfA(DebugBuffer,
                             MCHGR_DEBUG_PRINT_BUFF_LEN,
                             DebugMessage,
                             ap);
