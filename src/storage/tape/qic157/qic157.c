    ...
    )
{
    if ( (TapeClassCompareMemory(InquiryData->VendorId,"ECRIX",5) == 5) ||
         (TapeClassCompareMemory(InquiryData->VendorId,"EXABYTE",7) == 7) ) {

        if ( (TapeClassCompareMemory(InquiryData->ProductId,"VXA-1",5) == 5) ||
             (TapeClassCompareMemory(InquiryData->ProductId,"VXA-2",5) == 5) ) {
            return ECRIX_VXA_1;
        }
...

{
    ...
    if (CallNumber == 3) {
        ...
        __analysis_assume((sizeof(MODE_MEDIUM_PART_PAGE) - 4) == Srb->DataTransferLength);
        //
...

{
    ...
    if (CallNumber == 1) {
        ...
        __analysis_assume(sizeof(MODE_DATA_COMPRESS_PAGE) == Srb->DataTransferLength);
...

{
    ...
    if (CallNumber == 2) {
        ...
        __analysis_assume(sizeof(MODE_PARM_READ_WRITE_DATA) == Srb->DataTransferLength);
