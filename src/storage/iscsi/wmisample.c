...
--*/
{
    ...
    switch (GuidIndex) {
        case iSCSI_Operations_GUID_Index: {
            ...
            DebugPrint((iScsiPrtDebugTrace, "iSCSI Operations\n"));
        ...
        case iScsi_LB_Operations_GUID_Index: {
            ...
            DebugPrint((iScsiPrtDebugInfo, "Set LB policy\n"));
...
--*/
{
    ...
    ASSERT((IScsiSession->InitiatorID[0] == 0) &&
    ...
    DebugPrint((iScsiPrtDebugInfo, "Session Id returned - 0x%016I64x\n", retVal));
...
--*/
{
    ...
    convertedLength = wcslen(IScsiConnection->ISCSISession->TargetName) + 1;
    ...
    maxBufferSize = maxBufferSize - convertedLength;
    (PUCHAR)currentPos += (convertedLength * sizeof(WCHAR));
...
--*/
{
    ...
    rtlStatus = RtlStringCchCopyNW(InstanceName->Buffer,
                                   InstanceName->Length,
                                   uniCodeString,
                                   InstanceName->Length);
    ...
    )
{
    ...
    if ((iScsiStatus == STATUS_SUCCESS) && 
        ...
        if (!overFlow) {
            ...
            if (LBPolicy != NULL) {
                ...
                DoTraceMessage(iScsiDebugError,
                               "Failed to allocate memory for persisting LB policy\n");  
            }
        }
    }
    ...
    if (persistLBPolicy) {
        ...
        ASSERT(LBPolicy != NULL);

        iSpPersisteLBPolicy(AdapterExtension, LBPolicy);
    ...
    )
{
    ...
    if ((supportedLBPolicy->LoadBalancePolicy < MSiSCSI_LB_FAILOVER) ||
        ...
        DebugPrint((iScsiPrtDebugError, "Invalid Load Balance Policy %d\n",
...
--*/
{
    ...
    for (inx = 0; inx < SupportedLBPolicy->iSCSI_PathCount; inx++) {
        ...
        if (!found) {
            ...
            DebugPrint((iScsiPrtDebugError, "Path Id %I64x not found\n",
...
--*/
{
    ...
    for (inx = 0; inx < SupportedLBPolicy->iSCSI_PathCount; inx++) {
        ...
        if (iScsiPath->PrimaryPath) {
            ...
            if (!activeFound) {
                ...
                DebugPrint((iScsiPrtDebugInfo, "%p specified as Primary Path\n",
                            iScsiConnection));
    ...
    if (!activeFound) {
        ...
        DebugPrint((iScsiPrtDebugError, "No active path given for FO Only\n"));
...
--*/
{
    ...
    if (!activeFound) {
        ...
        DebugPrint((iScsiPrtDebugError, "No active path given for Round Robin.\n"));
    ...
    if (NT_SUCCESS(iScsiStatus)) {
        ...
        DebugPrint((iScsiPrtDebugInfo, "Successfully set LB policy\n"));
...
iSpLogError(
    ...
    __in_opt PUCHAR    ErrorData,
    IN ULONG           ErrorDataSize,
    __in_opt PWCHAR    TargetName,
    IN ULONG           ErrorCode
    )
/*+++
...
iSpFindPersistentTargetKey(
    IN PISCSI_ADAPTER_EXTENSION AdapterExtension,
    __in PWCHAR TargetName,
    IN PISCSI_TargetPortal TargetPortal,
    IN ULONG PortNumber,
    OUT PHANDLE TargetKey,
    __out_ecount_opt (TargetKeyNameSize) PWCHAR TargetKeyName, OPTIONAL
    ...
    if (NT_SUCCESS(status)) {
        ...
        while (!found) {
            ...
            if (NT_SUCCESS(status)) {
                ...
                //
                rtlStatus = RtlStringCchCopyNW(targetKeyName, nameLength,
                                               keyInfo->Name, nameLength);
                targetKeyName[255] = L'\0';
                            ...
                            if ((PortNumber == loginTargetIN->PortNumber) &&
                                ...
                                if (NT_SUCCESS(status)) {
                                    ...
                                    if (TargetKeyName != NULL) {
                                        ...
                                        ASSERT(TargetKeyNameSize > 0);

                                        rtlStatus = RtlStringCchCopyNW(TargetKeyName,
                                                                       TargetKeyNameSize,
                                                                       targetKeyName,
                                                                       TargetKeyNameSize);
                                    }
                ...
                DebugPrint((iScsiPrtDebugError, "Enumerated all persistent targets\n"));

                DoTraceMessage(iScsiDebugError, "Enumerated all persistent targets\n");
    ...
    )
{
    ...
    while (status == STATUS_SUCCESS) {
        ...
        if (NT_SUCCESS(status)) {
            ...
            rtlStatus = RtlStringCchCopyNW(subKeyName, nameLength,
                                           keyInfo->Name, nameLength);
            subKeyName[255] = L'\0';
            ...
            if (NT_SUCCESS(status)) {
                ...
                if (NT_SUCCESS(status)) {
                    ...
                    DebugPrint((iScsiPrtDebugInfo, "Deleted subkey %ws\n",
                                subKeyName));

                    DoTraceMessage(iScsiDebugTrace, "Deleted subkey %ws\n",
                                   subKeyName);
    ...
    if (!NT_SUCCESS(status) && (status == STATUS_NO_MORE_ENTRIES)) {
        ...
        if (!NT_SUCCESS(status)) {
            ...
            DoTraceMessage(iScsiDebugTrace,
                           "Successfully delete all keys and subkeys\n");
        }
    } else {
    ...
    )
{
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (NT_SUCCESS(status)) {
            ...
            for (inx = 0; inx < LBPolicy->ConnectionCount; inx++) {
                ...
                DebugPrint((iScsiPrtDebugInfo, "Will open subkey %ws\n",
                            subKeyName));
    ...
    )
{
    ...
    iSpFindPersistentTargetKey(AdapterExtension, 
                               targetName,
                               &(LoginTargetIn->TargetPortal),
                               LoginTargetIn->PortNumber,
                               &removeTargetKey,
                               targetKeyName, 
                               (sizeof(targetKeyName)/sizeof(WCHAR)));
    if (removeTargetKey != NULL) {
        ...
        if (suffixPtr != NULL) {
            ...
            DebugPrint((iScsiPrtDebugInfo, "Suffix is %ws\n", suffixPtr));
                ...
                if (wcsncmp(targetName, 
                    ...
                    while (connectionEntry != &iScsiSession->ConnectionList) {
                        ...
                        if (IsSamePortal(&targetPortal, &(LoginTargetIn->TargetPortal))) {
                            ...
                            DebugPrint((iScsiPrtDebugInfo, "Found session %p\n", 
                                        iScsiSession));
            ...
            DebugPrint((iScsiPrtDebugError,"No suffix found for target %ws\n",
                        targetKeyName));
        }
