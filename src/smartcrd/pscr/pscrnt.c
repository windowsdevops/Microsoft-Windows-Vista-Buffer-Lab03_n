   ...
   // parse all partial descriptors
    while (Count--) {
        ...
        case CmResourceTypeInterrupt: {
                ...
                DeviceExtension->bSharedIRQ = Shared;
...
--*/
{
    ...
    if (DeviceExtension->SmartcardExtension.VendorAttr.UnitNo < PSCR_MAX_DEVICE) {
   ...
   // delete the symbolic link
    ...
    if (DeviceExtension->SmartcardExtension.ReaderExtension != NULL) {
        ...
        ExFreePoolWithTag(DeviceExtension->SmartcardExtension.ReaderExtension, SMARTCARD_POOL_TAG);
...

{
    ...
    if (smartcardExtension->OsData->NotificationIrp) {
        ...
        SmartcardDebug(
                      DEBUG_TRACE,
                      ( "PSCR!PscrCleanup: Completing NotificationIrp %lx\n",
                        notificationIrp)
                      );
