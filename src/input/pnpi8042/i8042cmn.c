...
--*/
{
    ...
    {
        ULONG dataSize = 0, maxSize = 0;

        //
        // Our internal requests will only have 2 or 3 bytes, but a request from a filter
        // driver may have a longer buffer.  Make sure it is not too long, and if it is,
        // truncate the data we report in the event log
        //
        
        dataSize = commonData->CurrentOutput.ByteCount;
        maxSize = (ERROR_LOG_MAXIMUM_SIZE - sizeof(IO_ERROR_LOG_PACKET)) / sizeof(ULONG);

        if(commonData->CurrentOutput.ByteCount > maxSize){
            dataSize = maxSize;
        }
...
--*/
{
    ...
    if (timerContext.NewTimerCount == 0) {
        ...
        if(Globals.ReportResetErrors == TRUE)
        {
            ...
            //
            // Log a timeout error.  ERROR_LOG_MAXIMUM_SIZE
            //
            errorLogEntry = (PIO_ERROR_LOG_PACKET)IoAllocateErrorLogEntry(
                                                  DeviceObject,
                                                  (UCHAR) (sizeof(IO_ERROR_LOG_PACKET) +
