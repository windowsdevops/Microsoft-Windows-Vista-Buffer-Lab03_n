...

{
    ...
    for (i = 0; i < limit; i++) {
        ...
        //
        // It is possible for this to be called via I8xMouseEnableSyncRoutine, which is 
        // synced with the ISR and called at high IRQL.  In this case we cannot call
        // KeDelayExecutionThread, but must call KeStallExecutionProcessor.  There is a limit
        // on this loop so we will not hang the PC
        //

        if(irql > PASSIVE_LEVEL){

            KeStallExecutionProcessor(time);  // Delay in microseconds
        }
        else{
...

{
    ...
    NTSTATUS            status;
    ULONG               initFlags = 0;
    PPOWER_UP_WORK_ITEM item = (PPOWER_UP_WORK_ITEM)Context;
    ...
    __assume(mouseExtension != 0);      // annotation to avoid Prefast issue 28182
