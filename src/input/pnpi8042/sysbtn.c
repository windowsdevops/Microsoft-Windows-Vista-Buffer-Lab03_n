        ...
        if (KeyboardExtension->SysButtonEventIrp == NULL) {
            ...
            if(Irp->Cancel){
                ...
                if(IoSetCancelRoutine(Irp, NULL)){
                    //
                    // Cancel routine has not run yet. Complete the IRP here
                    //
                    ...
                    //
                    // Cancel routine has run and will complete the IRP when we drop
                    // our spinlock.  Return STATUS_PENDING so we don't touch the irp
                    //
                    IoMarkIrpPending(Irp);
                    status = STATUS_PENDING;
                }
