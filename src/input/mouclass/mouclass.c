...
#pragma alloc_text(PAGE,MouEnableDisablePort)
#endif
...
MouseQueryDeviceKey (
    ...
    __out_ecount(DataLength) PVOID   Data,
    IN  ULONG   DataLength
    )
{
    NTSTATUS        status;
    ...
    status = RtlULongAdd(sizeof(KEY_VALUE_FULL_INFORMATION), 
    ...
    status = RtlULongAdd(length, DataLength, &length);
    if(!NT_SUCCESS(status)){
        return status;
    }
    ...
    if (DeviceExtension->InputCount == 0) {
        ...
        if (Irp->Cancel) {
            ...
            if (oldCancelRoutine) {
                ...
                MouPrint((
                    0,
                    "MOUCLASS: Irp was cancelled, cancel Routine was NOT called"
                    ));
            }
            else {
                //
                ...
                MouPrint((
                    0,
                    "MOUCLASS: Irp was cancelled, cancel Routine was called"
                    ));
            }
        }
        ...
        if (status != STATUS_PENDING){
            //
            // The irp was cancelled but our cancel routine was not called, 
            // we need to complete it after releasing the spinlock
            //
            completeIrp = TRUE;
        }
        else{
...
MouCreateClassObject(
    ...
    __deref_out PWSTR            * FullDeviceName,
    IN  BOOLEAN             Legacy
    )
...

{
    ...
    *ClassDeviceObject = NULL;
    ...
    if (NULL == Globals.GrandMaster) {
        ...
        //
        // Make sure that BaseClassName.Length will not cause an integer
        // overflow.
        //
        if (Globals.BaseClassName.Length > MAXULONG - (sizeof(L"\\Device\\") + sizeof(L"0"))) {
            status = STATUS_UNSUCCESSFUL;
    ...
    if (!NT_SUCCESS(status) || NULL == (*ClassDeviceObject)) {
        MouPrint((
            1,
    ...
    *deviceExtension = *TmpDeviceExtension;
    ...
    IoInitializeRemoveLock (&deviceExtension->RemoveLock, MOUSE_POOL_TAG, 0, 0);
...

{
    ...
    if (DebugPrintLevel <= Globals.Debug) {
        ...
        if(NT_SUCCESS(RtlStringCchVPrintfA(buffer, 256, DebugMessage, ap))){      
            DbgPrint(buffer);
        }else{
...
MouseClassCreateWaitWakeIrpWorker (
    __in PDEVICE_OBJECT DeviceObject,
    __in_opt PMOUSE_WORK_ITEM_DATA  ItemData
    )
{
    PAGED_CODE ();
