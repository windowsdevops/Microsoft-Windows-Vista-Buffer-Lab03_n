    ...
    )
{
    ...
    if (gDriverObj != NULL)
    {
        ...
        if ((len > 0) && (len <= 255) && (len1 <= len) && (len2 <= len))
        {
            ...
            {
                TWarn(("Failed to allocate error log entry (len=%d).", len));
            }
        }
        else
        {
            TWarn(("Error log entry too big (len=%d).", len));
        }
    }
    ...
    )
{
    ...
    if (gDriverObj != NULL)
    {
        ...
        if (hr == S_OK)
        {
            hr = StringCchLengthA(szErrMsg, ARRAYSIZE(szErrMsg), &iLen);
        }
