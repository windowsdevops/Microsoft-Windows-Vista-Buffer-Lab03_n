    ...
    )
{
    ...
    if (DevExt->dwfHPen & HPENF_DEVICE_STARTED)
    {
        ...
        if (NT_SUCCESS(status))
        {
            status = SerialAsyncReadWritePort(FALSE, DevExt, "-1", 2);
            if (!NT_SUCCESS(status))
            {
    ...
    )
{
    ...
    status = SerialSyncReadWritePort(FALSE, DevExt, "0", 1, &lock, NULL, NULL);
    if (!NT_SUCCESS(status))
    {
    ...
    else
    {
        ...
        {
            status = SerialSyncReadWritePort(FALSE, DevExt, "*", 1, &lock, NULL, NULL);
            if (!NT_SUCCESS(status))
            {
                LogError(ERRLOG_QUERY_DEVINFO_FAILED,
                         status,
