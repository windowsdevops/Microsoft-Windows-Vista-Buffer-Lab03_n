...
/* ++
   ...
   FwpsAllocateNetBufferAndNetBufferList0. This function frees up 
...
/* ++
   ...
   via FwpsCloneStreamData0.
   
   FwpsCloneStreamData0 can return a chain of cloned NBLs; each NBL will 
   complete separately.
...
StreamOobEdit(
   ...
   IN FWPS_STREAM_DATA0* streamData,
   IN OUT FWPS_STREAM_CALLOUT_IO_PACKET0* ioPacket,
...
Exit:
   ...
   if (outgoingStreamData != NULL)
   {
      ...
      {
         FwpsFreeNetBufferList0(outgoingStreamData->netBufferList);
...
/* ++
   ...
   It reuses the FwpsCopyStreamDataToBuffer0 API (via StreamCopyDataForInspection)
   by creating a FWPS_STREAM_DATA0 struct.

-- */
{
   NTSTATUS status = STATUS_SUCCESS;

   FWPS_STREAM_DATA0 streamData = {0};
