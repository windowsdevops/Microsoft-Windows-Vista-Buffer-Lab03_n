   ...
   )
{
    ...
    memset(&sCallout, 0, sizeof(FWPS_CALLOUT0));
...
Arguments
    ...
    [IN] struct FWPS_FILTER0_*  filter      -   pointer to the Filter itself.
...
*/
{
   ...
   while (!IsListEmpty(&flowContextList))
   {
      ...
      status = FwpsFlowRemoveContext0(flowContext->flowHandle,
...
Arguments
   [IN] const FWPS_INCOMING_VALUES0* inFixedValues -  The fixed values passed in
                                                      based on the traffic.
   [IN] const FWPS_INCOMING_METADATA_VALUES0* inMetaValues - Metadata the
                                                             provides additional
                                                             information about the
                                                             connection.
   [IN] VOID* packet - Depending on the layer and protocol this can be NULL or a
                       layer specific type.
   [IN] const FWPS_FILTER0* filter - The filter that has specified this callout.

   [IN] UINT64 flowContext - Flow context associated with a flow
   [OUT] FWPS_CLASSIFY_OUT0* classifyOut - Out parameter that is used to inform
...
*/
{
   ...
   streamPacket = (FWPS_STREAM_CALLOUT_IO_PACKET0*) packet;
