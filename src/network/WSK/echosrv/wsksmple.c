...
(* PWSKSAMPLE_OP_HANDLER_FN)(
    __in PWSKSAMPLE_SOCKET_OP_CONTEXT SocketOpContext);
...
typedef struct _WSKSAMPLE_SOCKET_OP_CONTEXT {
    ...
    __field_bcount_part(BufferLength,DataLength) PVOID  DataBuffer;
    PMDL   DataMdl;
...
WskSampleAllocateSocketContext(
    __in PWSKSAMPLE_WORK_QUEUE WorkQueue,
    __in ULONG BufferLength
    );

VOID
WskSampleFreeSocketContext(
    __in PWSKSAMPLE_SOCKET_CONTEXT SocketContext
    );

VOID
WskSampleEnqueueOp(
    __in PWSKSAMPLE_SOCKET_OP_CONTEXT SocketOpContext,
    __in PWSKSAMPLE_OP_HANDLER_FN OpHandler
    );
...
WskSampleStartWorkQueue(
    __out PWSKSAMPLE_WORK_QUEUE WorkQueue
    );
    ...
    )
{
    ...
    for(i = 0; i < WSKSAMPLE_OP_COUNT; i++) {
        ...
        ASSERT(SocketContext->OpContext[i].OpHandler == NULL);
    ...
    )
{
    ...
    for(;;) {
        ...
        while(listEntry) {
            ...
            ASSERT(socketOpContext->OpHandler);
            socketOpContext->OpHandler = NULL; // purely debug aid
