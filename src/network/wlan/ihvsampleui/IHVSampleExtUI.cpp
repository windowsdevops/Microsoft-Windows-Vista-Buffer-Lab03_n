    ...
    case WM_INITDIALOG:
        {
            ...
            {
                SetDlgItemTextA(hwndDlg, IDC_EDIT_HELPER, pthis->m_pUIRequest->title);
            }
        }
        return TRUE;
...
HRESULT CDot11SampleExtUI::GetClsidPropertyName (
                      const CLSID* pCLSID, 
                      __in_opt __nullterminated const wchar_t *pwszPropertyName,
                      __writableTo(byteCount(maxResultLen * sizeof(wchar_t))) wchar_t *pwszResultStr,
                      UINT  maxResultLen)
{
