...
#pragma prefast(suppress: __WARNING_POTENTIAL_WRITE_OVERRUN, "path is of length RegistryPath->Length+sizeof(WCHAR) bytes")        
    ...
    )
{
    ...
    //  If fakemodem sample is installed as a root-enumerated device,
    //  "PortName" registry key doesn't exist. Therefore check for the 
    //  return value.

    status = FakeModemHandleSymbolicLink(Pdo, TRUE, DeviceExtension, Fdo);
    ...
    )
{
    ...
    //
    TempLength = (ULONG64)sizeof(KEY_VALUE_FULL_INFORMATION) + (ULONG64)KeyNameStringLength  + (ULONG64)DataLength;
    if (TempLength > (ULONG)-1)
    {
...

{
    ...
    RtlZeroMemory(ComPort, sizeof(ComPort));
