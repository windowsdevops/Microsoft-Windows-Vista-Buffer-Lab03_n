...

{
    ...
    if ((DeviceExtension->CurrentReadIrp != NULL) && (DeviceExtension->BytesInReadBuffer > 0)) {
        ...
        BytesToMove %= (READ_BUFFER_SIZE+1); //BytesToMove should be limited to values form 0 to READ_BUFFER_SIZE inclusive

#pragma prefast(suppress:__WARNING_INDEX_WRONG_OVERFLOW_CHECK,"Bogus warning due to prefast tool bug")
...
#pragma prefast(suppress:__WARNING_INDEX_UNDERFLOW,"Bogus warning due to prefast tool bug")
