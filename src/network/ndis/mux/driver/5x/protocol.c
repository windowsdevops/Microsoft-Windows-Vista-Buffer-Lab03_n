...
--*/
{
    ...
    do
    {
        ...
        for (i = 0; i < MAX_VELAN_NUMBER_BYTES; i++)
        {
            if (pAdapt->AllocatedVelanNumber[i] != 0xff)
            {
                k = 1;
                for (j = 0; j < 8; j++)
                {
                    if ((pAdapt->AllocatedVelanNumber[i] & k) == 0)
                    {
                        pAdapt->AllocatedVelanNumber[i] |=  k;
                        VelanNumber += j;
                        break;
                    }
                    k <<= 1;
        ...
        if (VelanNumber == MAX_VELAN_NUMBER_PER_ADAPT)
        {
            Status = NDIS_STATUS_RESOURCES;
        ...
        pVElan->VElanNumber = VelanNumber + 1;
...
--*/
{
    ...
    MUX_ACQUIRE_ADAPT_WRITE_LOCK(pAdapt, &LockState);
    //
    // Free the VelanNumber allocated to this VELAN
    //
    ASSERT(pVElan->VElanNumber);
    ByteIndex = (pVElan->VElanNumber - 1) / 8;
                
    ASSERT(MAX_VELAN_NUMBER_BYTES >= (ByteIndex + 1));
    pAdapt->AllocatedVelanNumber[ByteIndex] &= ~(1 << ((pVElan->VElanNumber - 1) % 8));   
...
--*/
{
    ...
    do
    {
        ...
        // ETH_HEADER_SIZE + VLAN_TAG_HEADER_SIZE
        // 
        if (BufferLength < ETH_HEADER_SIZE + VLAN_TAG_HEADER_SIZE)
        {
            break;
        }
