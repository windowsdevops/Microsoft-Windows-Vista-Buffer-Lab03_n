    ...
    */
    ...
    while (!IsListEmpty(&pAdapter->EventList))
    {
        ...
        case CARD_EVENT_RING:
            ...
            {
                //
                // The call setup has failed. This means we cannot rely on
                // the pBChannel to be set to the Channel anymore (it is most
                // likely NULL). Restore it before rejecting the call.
                //
                pBChannel = pEvent->pReceivingObject;
...
USHORT CardCleanPhoneNumber(
    __out_ecount(CARD_MAX_DIAL_DIGITS+1) PUCHAR   Dst,                        // @parm
    // A pointer to the output string.

    __in_ecount(Length) PUSHORT                   Src,                        // @parm
