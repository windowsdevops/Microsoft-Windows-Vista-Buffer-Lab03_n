    ...
    )
{
    ...
    */
    ...
    if (Status == NDIS_STATUS_SUCCESS)
    {
        ...
        //check for integer owerflow
        ASSERT((g_MemoryAllocated + dwSize) > g_MemoryAllocated);

        if ((g_MemoryAllocated + dwSize) < g_MemoryAllocated)
        {
            DBG_ERROR(DbgInfo,("Counter for Total Memory Allocated exceeded MAX_ULONG: %d + %d\n",
    ...
    )
{
    ...
    */
    ...
    if (*pVirtualAddress)
    {
        ...
        ASSERT((g_SharedMemoryAllocated + dwSize)> g_SharedMemoryAllocated);
        if ((g_SharedMemoryAllocated + dwSize) < g_SharedMemoryAllocated)
        {
            DBG_ERROR(DbgInfo,("Counter for Total Shared Memory Allocated exceeded MAX_ULONG: %d + %d\n",
