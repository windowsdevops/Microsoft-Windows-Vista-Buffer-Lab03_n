    ...
    )
{
    ...
    */
    ...
    case OID_PNP_CAPABILITIES:
        ...
        // Please make sure that the Flags field in the InformationBuffer
        // passed down remains unchanged. Miniport driver is not supposed
        // to edit this field
        if(InformationBufferLength >= sizeof(NDIS_PNP_CAPABILITIES))
        {
            PNDIS_PNP_CAPABILITIES InputPnPCaps;
            InputPnPCaps = (PNDIS_PNP_CAPABILITIES) InformationBuffer;
            PnpCapabilities.Flags = InputPnPCaps->Flags;
        }
        else
        {
            DBG_ERROR(pAdapter,("OID_PNP_CAPABILITIES: Invalid size=%d expected=%d\n",
                InformationBufferLength, SourceBufferLength));
        }
        break;
...
DBG_STATIC NDIS_STATUS MiniportCoSetInformation(
    ...
    __in_bcount(InformationBufferLength) IN PVOID   InformationBuffer,          // @parm
