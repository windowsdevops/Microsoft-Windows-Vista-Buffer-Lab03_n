          ...
          CurrentNetBuffer = NET_BUFFER_NEXT_NB(CurrentNetBuffer))
    {
        ...
        if (pMpTxd->TotalSGRequested < MP_MAX_NUM_OF_FRAGMENTS)
        {
            ...
            ndisStatus = NdisMAllocateNetBufferSGList(
                ...
                (pMpTxd->ScatterGatherList + (pAdapter->ScatterGatherListSize * pMpTxd->TotalSGRequested)),
    ...
    )
{
    ...
    do
    {
        ...
        // Integer overflow
        if (pAdapter->TotalTxMSDUs * MP_MAX_NUM_OF_FRAGMENTS > 
            pAdapter->TotalTxMSDUs * MP_MAX_NUM_OF_FRAGMENTS * pAdapter->ScatterGatherListSize)
        {
        ...
        for (i=0; i < pAdapter->TotalTxMSDUs; i++)
        {
            ...
            //
            pCurrentMpTxd->ScatterGatherList = pAdapter->ScatterGatherListBuffers + (i * pAdapter->ScatterGatherListSize);
