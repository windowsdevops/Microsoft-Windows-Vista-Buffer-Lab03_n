...
typedef struct _ADAPTER
{
    ...
    /** Count of Scatter Gather operations pending on the OS */
    ULONG                               PendingSGOperations;

    /** This event is set when all SG operations on miniport are done */
    NDIS_EVENT                          SGOperationsDoneEvent;
    ...
    /** Scatter Gather list buffers allocated for sending */
    ...
    NDIS_HANDLE                         NdisResetWorkItem;

    /** Dot11 Reset can get issued at Dispatch level */
    NDIS_HANDLE                         Dot11ResetWorkItem;
...
#define     MP_SET_STATUS_FLAG(_M, _F)                  MP_INTERLOCKED_SET_BITS(&((_M)->Status), (_F))
#define     MP_CLEAR_STATUS_FLAG(_M, _F)                MP_INTERLOCKED_CLEAR_BITS(&((_M)->Status), (_F))
...
#define MP_INCREMENT_ADAPTER_PENDING_SG_OP(_Adapter)            \
{                                                               \
    if (NdisInterlockedIncrement((PLONG)&_Adapter->PendingSGOperations) == 1)   \
        NdisResetEvent(&_Adapter->SGOperationsDoneEvent);                       \
}

#define MP_DECREMENT_ADAPTER_PENDING_SG_OP(_Adapter)            \
{                                                               \
    if (NdisInterlockedDecrement((PLONG)&_Adapter->PendingSGOperations) == 0)   \
        NdisSetEvent(&_Adapter->SGOperationsDoneEvent);                         \
}
...
{ \
    Hw11CleanupNetBufferList((_Adapter)->pNic, _NBL); \
...
#define MP_ACQUIRE_SEND_LOCK(_Adapter, _DispatchLevel)      MP_ACQUIRE_SPIN_LOCK(&(_Adapter->SendLock), _DispatchLevel)
#define MP_RELEASE_SEND_LOCK(_Adapter, _DispatchLevel)      MP_RELEASE_SPIN_LOCK(&(_Adapter->SendLock), _DispatchLevel)
...
#define MP_NIC_POWER_STATE_IS_OFF(_Adapter)  (!Hw11QueryNicPowerState(_Adapter->pNic, FALSE))
