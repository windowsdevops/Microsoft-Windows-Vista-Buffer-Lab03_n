    ...
    )
{
    ...
    else
    {
        ...
        //
        NdisMCancelTimer(&pStation->AdHocStaInfo.WatchdogTimer, &cancelled);
        if (!cancelled) 
        {
    ...
    )
{
    ...
    __try 
    {
        ...
        // Integer overflow
        if ((FIELD_OFFSET(DOT11_BSS_DESCRIPTION, ucBuffer) + StaEntry->InfoElemBlobSize) > 
            (FIELD_OFFSET(DOT11_BSS_DESCRIPTION, ucBuffer) + StaEntry->InfoElemBlobSize + 3))
        {
    ...
    )
{
    ...
    if (connected)
    {
        Hw11StartAdHoc(pStation->pNic, NULL);
    }
}
...
StaGetMatchingAdHocStaList(
    IN  PSTATION pStation,
    OUT PULONG StaCount,
    OUT PSTA_ADHOC_STA_ENTRY **StaEntryArray
    )
{
    NDIS_STATUS             ndisStatus = NDIS_STATUS_SUCCESS;
    PLIST_ENTRY             pHead = NULL, pEntry = NULL;
    ...
    )
{
    ...
    UNREFERENCED_PARAMETER(pvSystemSpecific1);
    UNREFERENCED_PARAMETER(pvSystemSpecific2);
    UNREFERENCED_PARAMETER(pvSystemSpecific3);
