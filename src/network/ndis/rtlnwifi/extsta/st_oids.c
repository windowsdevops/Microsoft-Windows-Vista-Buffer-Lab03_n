...
Revision History:
    ...
    ----------    ----------------------------------------------
    08-01-2005    Created
    01-15-2006    Renamed sta_oids.c to st_oids.c
    ...
    )
{
    ...
    do
    {
        ...
        if (FIELD_OFFSET(DOT11_MAC_ADDRESS_LIST, MacAddrs) > 
                ...
                pStation->Config.ExcludedMACAddressCount * sizeof(DOT11_MAC_ADDRESS))
        {
    ...
    )
{
    ...
    do
    {
        ...
        if (FIELD_OFFSET(DOT11_BSSID_LIST, BSSIDs) > 
                ...
                pStation->Config.DesiredBSSIDCount * sizeof(DOT11_MAC_ADDRESS))
        {
            return NDIS_STATUS_FAILURE;
        }
...
StaGetAlgorithmPair(
    IN  PSTATION                    pStation,
    IN  STA_QUERY_ALGO_PAIR_FUNC    QueryFunction,
    OUT PDOT11_AUTH_CIPHER_PAIR    *AlgoPairs,
    OUT PULONG                      NumAlgoPairs
    )
{
...
Sta11QueryUnicastUseGroupOption(
    IN  PSTATION        pStation,
    OUT PBOOLEAN        unicastUseGroupEnabled
    )
{
...
Sta11QuerySafeModeOption(
    IN  PSTATION        pStation,
    OUT PBOOLEAN        safeModeEnabled
    )
{
    *safeModeEnabled = pStation->Config.SafeModeEnabled;
    return NDIS_STATUS_SUCCESS;
}
...
Sta11QueryHiddenNetworkOption(
    IN  PSTATION        pStation,
    OUT PBOOLEAN        hiddenNetworkEnabled
    )
{
...
Sta11QueryMediaStreamingOption(
    IN  PSTATION        pStation,
    OUT PBOOLEAN        mediaStreamEnabled
    )
{
    ...
    )
{
    ...
    else 
    {
        ...
        return StaSetPowerSavingLevel(pStation, PowerSavingLevel);
    }
}
    ...
    )
{
    ...
    if (ndisStatus == NDIS_STATUS_SUCCESS)
    {
        pStation->Config.SafeModeEnabled = safeModeEnabled;
    }
