...
StaGetRSNOUIFromCipher(
    IN ULONG prefix,
    IN DOT11_CIPHER_ALGORITHM cipher
    )
{
    switch (cipher)
    {
    ...
    )
{
    ...
    {
        size += sizeof(USHORT) + ((USHORT)pStation->Config.PMKIDCount) * sizeof(DOT11_PMKID_VALUE);
                                                        // PMKID
    }
    ...
    *ppCurrentIE += sizeof(USHORT);
    ...
    if (pStation->Config.AuthAlgorithm == DOT11_AUTH_ALGO_RSNA &&
        pStation->Config.PMKIDCount > 0)
    {
        *((USHORT UNALIGNED *)(*ppCurrentIE)) = (USHORT)pStation->Config.PMKIDCount;
        *ppCurrentIE += sizeof(USHORT);

        for (index = 0; index < pStation->Config.PMKIDCount; index++)
        {
            NdisMoveMemory(*ppCurrentIE,
                           pStation->Config.PMKIDList[index].PMKID,
