    ...
    )
{
    ...
    do
    {
        ...
        for (i=0; i<(ULONG)pRecvContext->usNumberOfMPDUsReceived; i++)
        {
            ...
            //
            Hw11ReturnFragment(pAdapter->pNic, pMpRxd->Fragments[i], DispatchLevel);
        }
    ...
    )
{
    ...
    //
    if (!Hw11FilterFragment(pAdapter->pNic, pNicFragment))
    {
...
MpAddRxFragmentToMSDU(
    ...
    IN  USHORT                      usFragmentSize,
    IN  UCHAR                       ucFragNumber
    )
{
    ...
    do
    {
        ...
        {
            MpTrace(COMP_RECV, DBG_LOUD,  ("Duplicate packet received!!!\n"));
    ...
    )
{
    ...
    do
    {
        ...
        for (i=1; i<pMpRxd->FragmentCount; i++)
        {
            ...
            MP_RX_FRAGMENT_SET_OFFSET(pMpFragment, (HeaderLength + Hw11GetFragmentDataOffset(pMpRxd->Fragments[i])));
        ...
        //
        Hw11FillReceiveContext(pAdapter->pNic, pLastFragment, &pMpRxd->Dot11RecvContext);        
    ...
    )
{
    ...
    // Allow hardware to fill in the relevant Receive context
    //
    Hw11FillReceiveContext(pAdapter->pNic, pMpRxd->Fragments[0], &pMpRxd->Dot11RecvContext);        
...
MpFindPartialMSDUInReassemblyLine(
    IN PADAPTER     pAdapter,
    IN PMP_DOT11_MGMT_DATA_MAC_HEADER pFragmentHdr, 
    IN USHORT       usSequenceNumber
    )
{
    PMP_RX_MSDU pMpRxd;
    USHORT      i;
...
Mp11AssociateRxFragment(
    IN  PADAPTER         pAdapter,
    IN  PNIC_RX_FRAGMENT Fragment,
    OUT PMP_RX_FRAGMENT* ppMpFragment
    )
{
    NDIS_STATUS     ndisStatus;
    PVOID           pSysVa;
    ...
    {
        pMpFragment = NULL;
        pSysVa = Hw11GetVirtualAddress(Fragment);
        MPVERIFY(pSysVa);
        ...
        //
        // Allocate the MDL this fragment will use
        //
        pMpFragment->Mdl = MP_ALLOCATE_MDL(pSysVa, MP_802_11_MAX_FRAME_SIZE);
        if (pMpFragment->Mdl == NULL)
        {
            MpTrace(COMP_RECV, DBG_SERIOUS,  ("Failed to allocate MDL from OS\n"));
            ndisStatus = NDIS_STATUS_RESOURCES;
            break;
        }
        ...
        //
        MP_MAP_MDL_PHYSICAL_PAGES(pMpFragment->Mdl);
    ...
    if (ndisStatus != NDIS_STATUS_SUCCESS)
    {
        ...
        {
            if (pMpFragment->Mdl)
                MP_FREE_MDL(pMpFragment->Mdl);
            if (pMpFragment->NetBuffer)
    ...
    )
{
    ...
    //
    // Free the Fragment and its associated MDL and NetBuffer
    //
    MP_FREE_MDL(pMpFragment->Mdl);
    NdisFreeNetBuffer(pMpFragment->NetBuffer);
    MP_FREE_MEMORY(pMpFragment);
    ...
    )
{
    ...
    USHORT              usFragmentSize, usFragmentOffset;
    PMP_RX_MSDU         pMpRxd = NULL;
    PNIC                pNic = pAdapter->pNic;
            ...
            Hw11ReceiveIsAvailable(pNic))
    {
        ...
        //
        pNicFragment = Hw11GetReceivedFragment(pNic, TRUE);
        ...
        MP_RX_FRAGMENT_SET_OFFSET(pMpFragment, usFragmentOffset);
    ...
    {
        MpTrace(COMP_RECV, DBG_TRACE,  ("Countdown expired! Cleaning up any orphan fragments from the Reassembly line\n"));
    ...
    //
    Hw11ReceiveInterruptComplete(pAdapter->pNic);
    ...
    )
{
            ...
            Hw11ReceiveIsAvailable(pNic))
    {
        ...
        //
        if (!Hw11FragmentIsGood(pAdapter->pNic, pNicFragment))
        {
        ...
        if (ndisStatus == NDIS_STATUS_SUCCESS)
        {
            ...
            //
            ndisStatus = Hw11SecurityCheck(pAdapter->pNic, 
