...
#endif
    {NDIS_STRING_CONST("*ReceiveBuffers"),  0, MP_OFFSET(NumRfd),           MP_SIZE(NumRfd),            128,                     NIC_MIN_RFDS,   NIC_MAX_RFDS},
    {NDIS_STRING_CONST("*TransmitBuffers"), 0, MP_OFFSET(NumTcb),           MP_SIZE(NumTcb),            NIC_DEF_TCBS,           1,              NIC_MAX_TCBS},
...
--*/    
{
    ...
    CommonConfigCharBuf buffer[NIC_PCI_E100_HDR_LENGTH ];
...
--*/    
{
    ...
    do
    {
        ...
        //
        SgListLength = (ULONGLONG)Adapter->NumTcb * (ULONGLONG)Adapter->ScatterGatherListSize;
...
--*/    
{
    ...
    do
    {
        ...
        for (RfdCount = Adapter->NumRfd; RfdCount > NIC_MIN_RFDS; RfdCount >>= 1)
        {
            ...
            // Integer overflow check
            //
            RfdLength = (ULONGLONG)Adapter->HwRfdSize * (ULONGLONG)RfdCount;
...
--*/    
{
    ...
    do
    {
        ...
        // 
        pMpRfd->Mdl = NdisAllocateMdl(Adapter->AdapterHandle, (PVOID)&pHwRfd->RfdBuffer.RxMacHeader, NIC_MAX_PACKET_SIZE);
