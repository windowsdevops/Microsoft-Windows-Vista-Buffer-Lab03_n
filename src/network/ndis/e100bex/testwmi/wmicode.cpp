  ...
  if ( hr == WBEM_S_NO_ERROR ) {
     ...
     while ( (hr == WBEM_S_NO_ERROR) && (ulFound == 1) ) {
        ...
        if ( hr == WBEM_S_NO_ERROR ) {
           ...
           hrTemp = VariantClear( &varInstanceName );
           ASSERT(hrTemp == S_OK);
                     ...
                     HWND          hwndPropTree)
{
  ...
  if ( pInst ) {
     ...
     if ( hr == WBEM_S_NO_ERROR ) {
        ...
        hrTemp = SafeArrayGetLBound( psaPropNames, 1, &lLower );
        ASSERT(hrTemp == S_OK);
        hrTemp = SafeArrayGetUBound( psaPropNames, 1, &lUpper );
        ASSERT(hrTemp == S_OK);
        ...
        hr = SafeArrayDestroy( psaPropNames );
        ASSERT(hr == S_OK);
     }
     else {
        PrintError( hr,
