...
MPIssueScbPoMgmtCommand(
    ...
    __in PCSR_FILTER_STRUC pFilter,
...
MPCreateProgrammableFilter (
    __in PMP_WAKE_PATTERN     pMpWakePattern ,
    __inout PULONG pFilter,
    __inout PULONG pNext
    );
...
MPIssueScbPoMgmtCommand(
    __in PFDO_DATA FdoData,
    __in PCSR_FILTER_STRUC pNewFilter,
    __in BOOLEAN WaitForScb
    )
{
...
#ifdef _PREFAST_
#pragma prefast (push)
#pragma prefast (disable: __WARNING_INCORRECT_VALIDATION)     // bogus "Potential overflow using expression "
#pragma prefast (disable: __WARNING_INCORRECT_ANNOTATION)     // bogus "Potential overflow using expression "
#endif /* _PREFAST_ */

NTSTATUS
MPCalculateE100PatternForFilter (
    __in_bcount(FrameLength) PUCHAR pFrame,
    __in ULONG FrameLength,
    __in_bcount(MaskLength) PUCHAR pMask,
    __in ULONG MaskLength,
    __out PULONG pSignature
    )
/*++
Routine Description:
...
--*/
{
    PUCHAR pCurrentByte = (PUCHAR)pFilter;
...
MPAreTwoPatternsEqual(
    __in PNDIS_PM_PACKET_PATTERN pNdisPattern1,
    __in PNDIS_PM_PACKET_PATTERN pNdisPattern2
    )
/*++
Routine Description:
...
NICAddWakeUpPattern(
    ...
    __in UINT         InformationBufferLength,
...
--*/
{
    ...
    do
    {
        ...
        status = RtlULongAdd(pPmPattern->PatternOffset,
                             pPmPattern->PatternSize,
                             &CopyLength);
        if(!NT_SUCCESS(status)){
            break; // overflow error
        }

        if (InformationBufferLength < CopyLength)
        {
            status = STATUS_BUFFER_TOO_SMALL;
...
--*/
{
    ...
    do
    {
        ...
        status = RtlULongAdd(pReqPattern->PatternOffset,
                             pReqPattern->PatternSize,
                             &result);
        if(!NT_SUCCESS(status)){
            break; // overflow error
        }

        if (InformationBufferLength < result)
        {
            status = STATUS_BUFFER_TOO_SMALL;
