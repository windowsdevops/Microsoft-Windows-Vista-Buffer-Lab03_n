                ...
                APO_CONNECTION_DESCRIPTOR** ppOutputConnections)
{
    ...
    _ASSERTE(UncompOutputFormat.dwSamplePerFrame == UncompInputFormat.dwSamplePerFrame);
    _ASSERTE(UncompOutputFormat.f32SamplingRate == UncompInputFormat.f32SamplingRate);
...
(
    UINT                nFormat,
    __deref_out LPWSTR* ppwstrFormatRep
)
{
    HRESULT hr;
    size_t  cbRep;
    ...
    *ppwstrFormatRep = (LPWSTR)CoTaskMemAlloc(cbRep);
    IF_TRUE_ACTION_JUMP((*ppwstrFormatRep == NULL), hr = E_OUTOFMEMORY, Exit);

    hr = StringCbCopyW(*ppwstrFormatRep, cbRep, _rgCustomFormats[nFormat].pwszRep);
