...

{
    ...
    CEncoderDevice *EncDevice = new (NonPagedPool, MS_SAMPLE_ANALOG_POOL_TAG) CEncoderDevice (Device);
    ...
    if (!m_Device -> Started) {
 	...
 	if (!m_HardwareSimulation) {
	    ...
	    Status = KsAddItemToObjectBag (
		       ...
		       reinterpret_cast <PFNKSFREE> (CEncoderDevice::CleanupHW)
		       );	
	...
	0) == 0) {
        ...
        if (NT_SUCCESS (Status)) {
            ...
            Status = KsAddItemToObjectBag (
                m_Device -> Bag,
                m_TsSynth,
                reinterpret_cast <PFNKSFREE> (CEncoderDevice::CleanupSynth)
                );
    ...
    )
{
  ...
  if (m_TsSynth) m_TsSynth->SetFrequency(ulFrequency);
}
