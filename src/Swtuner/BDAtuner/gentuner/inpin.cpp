...
/*    Status = KsPinGetConnectedFilterInterface(Pin,&IID_IKsControl,(PVOID *) &control);
    if (NT_SUCCESS(Status)) {
        KSPROPERTY property;
        ULONG bytesReturned;
        
        property.Set = KSPROPSETID_BdaFrequencyFilter;
        property.Id = KSPROPERTY_BDA_RF_TUNER_FREQUENCY;
