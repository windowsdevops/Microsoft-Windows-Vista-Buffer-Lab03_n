...

{
    ...
    if (!CapDevice) {
		...
		guid=(GUID *)&KSNAME_CaptureNodePropertySet;
		Status=IoRegisterDeviceInterface(Device->PhysicalDeviceObject,guid,NULL, &SymbolicLinkName);
		if (NT_SUCCESS(Status)) {
			DbgPrint( "Registered Device Interfaces");
			Status=IoSetDeviceInterfaceState(&SymbolicLinkName,TRUE);
			if (!NT_SUCCESS(Status)) {
					DbgPrint("IoSetInterfaceState err=%ld\n", Status);
				Status = STATUS_SUCCESS;
			}
			else
			{
				DbgPrint( "IoSetDeviceInterfaceState Passed");			//
			}
		} else {
			DbgPrint( "Registered Device Interfaces Failed err = %x", Status);			//
			// its ok if this call failed...
			//
			Status = STATUS_SUCCESS;
		}
    }
...
{
	KIRQL  oldIrql;
    DbgPrint("CCaptureDevice::SetFrequency");

	KeAcquireSpinLock (&frequencyLock, &oldIrql);
	m_ulFrequency = *pulFrequency;

	if (m_TsSynth) 
		m_TsSynth->SetFrequency(m_ulFrequency);

	KeReleaseSpinLock (&frequencyLock, oldIrql);
	return STATUS_SUCCESS;
	}

ULONG CCaptureDevice::Get_theFrequency()
{
...

{
    ...
    g_RegistryPath.Buffer = (PWSTR) ExAllocatePoolWithTag(NonPagedPool, RegistryPath->MaximumLength, MS_SAMPLE_ANALOG_POOL_TAG);
    g_RegistryPath.Length = RegistryPath->Length;
    g_RegistryPath.MaximumLength = RegistryPath->MaximumLength;
    RtlMoveMemory(g_RegistryPath.Buffer, RegistryPath->Buffer, RegistryPath->MaximumLength);
