...
#ifdef ALLOC_PRAGMA
#pragma alloc_text(PAGE, CdAddAllocation)
...
Routine Description:
    ...
    This routine is called to add an entry into the Cd Mcb.  We grow the Mcb
    as necessary and update the new entry.  This routine handles the single
    entry for a directory file.  We will round the start block down to a
    sector boundary.  Our caller has already biased the DataLength with
    any adjustements.  This is used for the case where there is a single
    entry and we want to align the data on a sector boundary.
