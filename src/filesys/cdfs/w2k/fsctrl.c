...
#pragma alloc_text(PAGE, CdLockVolumeInternal)
...
#pragma alloc_text(PAGE, CdUnlockVolumeInternal)
...

{
    ...
    if (FlagOn( Vcb->VcbState, VCB_STATE_LOCKED )) {
        ...
        if (Vcb->VcbUserReference == CDFS_RESIDUAL_USER_REFERENCE + RemainingUserReferences) {
...

{
    ...
    if (Irp->AssociatedIrp.SystemBuffer != NULL) {
        ...
        if (NULL == VolumeState) {
            ...
            CdCompleteRequest( IrpContext, Irp, STATUS_INSUFFICIENT_RESOURCES );
    ...
    while (Links != &CdData.VcbQueue) {
        ...
        if (Vcb->Vpb->RealDevice == DeviceToMarkBad) {
            ...
            if (DeviceToMarkBad->Vpb == Vcb->Vpb) {
                ...
                PVPB NewVpb = ExAllocatePoolWithTag( NonPagedPoolMustSucceed, sizeof( VPB ), TAG_VPB );
