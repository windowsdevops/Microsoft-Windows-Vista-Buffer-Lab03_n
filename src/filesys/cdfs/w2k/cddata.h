...
extern PFN_FSRTLTEARDOWNPERSTREAMCONTEXTS CdFsRtlTeardownPerStreamContexts;
...
#define DebugBreakOnStatus(S) {                                                      \
    if (CdTestRaisedStatus) {                                                        \
        if ((S) == STATUS_DISK_CORRUPT_ERROR || (S) == STATUS_FILE_CORRUPT_ERROR) {  \
            DbgPrint( "CDFS: Breaking on interesting raised status (%08x)\n", (S) ); \
            DbgPrint( "CDFS: Set CdTestRaisedStatus @ %08x to 0 to disable\n",       \
                      &CdTestRaisedStatus );                                         \
            DbgBreakPoint();                                                         \
        }                                                                            \
    }                                                                                \
...
#define ASSERT_EXCLUSIVE_CDDATA             ASSERT( ExIsResourceAcquiredExclusive( &CdData.DataResource ))
