...
CdPrepareBuffers (
    ...
    __in PIRP Irp,
    __in PFCB Fcb,
    __in_bcount(ByteCount) PVOID UserBuffer,
    __in ULONG UserBufferOffset,
    __in LONGLONG StartingOffset,
    __in ULONG ByteCount,
    __out PIO_RUN IoRuns,
    __out PULONG RunCount,
    __out PULONG ThisByteCount
    );
...
CdFinishBuffers (
    __in PIRP_CONTEXT IrpContext,
    __inout PIO_RUN IoRuns,
    __in ULONG RunCount,
    __in BOOLEAN FinalCleanup,
    __in BOOLEAN SaveXABuffer
    );
...
CdMultipleXAAsync (
    ...
    __in ULONG RunCount,
    __inout PIO_RUN IoRuns,
    __in PRAW_READ_INFO RawReads,
    __in TRACK_MODE_TYPE TrackMode
    );
...

{
    ...
    BOOLEAN Exclusive;
    ULONG HitBufferBaseLbn;
    PUCHAR HitBufferBase;
#if DBG
        ...
        while (Remaining) {
            ...
            if (NULL != Buffer) {
                ...
                if (!JustRead) {
                    ...
                    InterlockedIncrement( &Vcb->SecCacheHits);
                }
            ...
            if (STATUS_PENDING == Status)  {
                ...
                KeWaitForSingleObject( &Vcb->SectorCacheEvent,
                                       Executive,
                                       KernelMode,
                                       FALSE,
                                       NULL );
...

{
    ...
    for (UnwindRunCount = 0;
        ...
        if ((SafeNodeType( Fcb) == CDFS_NTC_FCB_INDEX) &&
            ...
            CdReadDirDataThroughCache( IrpContext, &IoRuns[ UnwindRunCount]);
            UsedCache = TRUE;
            continue;
        }
...

{
    ...
    if ((SafeNodeType( Fcb) == CDFS_NTC_FCB_INDEX) &&
        ...
        CdReadDirDataThroughCache( IrpContext, Run);
