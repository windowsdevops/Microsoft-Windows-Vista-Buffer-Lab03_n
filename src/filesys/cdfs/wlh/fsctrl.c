...
CdReMountOldVcb (
    ...
    __inout PVCB OldVcb,
    __inout PVCB NewVcb,
    __in PDEVICE_OBJECT DeviceObjectWeTalkTo
    );
...
CdFindPrimaryVd (
    __in PIRP_CONTEXT IrpContext,
    __inout PVCB Vcb,
    __out_bcount(SECTOR_SIZE) PCHAR RawIsoVd,
    __in ULONG BlockFactor,
    __in BOOLEAN ReturnOnError,
    __in BOOLEAN VerifyVolume
    );
...
CdFindActiveVolDescriptor (
    __in PIRP_CONTEXT IrpContext,
    __in PVCB Vcb,
    __inout_bcount(ROUND_TO_PAGES( SECTOR_SIZE )) PCHAR RawIsoVd,
    __in BOOLEAN VerifyVolume
    );
