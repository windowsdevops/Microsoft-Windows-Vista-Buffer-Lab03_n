...
HANDLE gKtmTransactionManagerHandle = NULL;

HANDLE gKtmResourceManagerHandle = NULL;

PKRESOURCEMANAGER gKtmResourceManager = NULL;

NPAGED_LOOKASIDE_LIST gTransactionList;

UNICODE_STRING gNtfsDriverName = RTL_CONSTANT_STRING(L"\\FileSystem\\Ntfs");
...
#if WINVER >= 0x0600
#pragma alloc_text(PAGE, SpyKtmNotification)
#endif
#endif
...
//
    ...
    //
    //  Create a global resource manager (and a transaction manager) across
    //  Filespy.
    //
...
#   define DEVOBJ_LIST_SIZE 64
    ...
    ExDeleteNPagedLookasideList( &gTransactionList );
#endif
...
--*/
{
    ...
    status = IoCallDriver( devExt->NLExtHeader.AttachedToDeviceObject,
                           Irp );
    ...
    if (syncToDispatch) {
        ...
        //
        //  Check if a transaction is bound to this Irp.
        //  SpyCheckTransaction will verify it is a IRP_MJ_CREATE and
        //  enlist in the transaction if it has not done yet.
        //

        if (NT_SUCCESS( status ) &&
            FlagOn( devExt->Flags, IsAttachedToNTFS )) {

            //
            //  As of today, only NTFS supports transactions.
            //

            SpyCheckTransaction( Irp );
        }
#endif
...
#   define MVInsufResMsg "FileSpy!SpyFsControlMountVolume: Error getting device name, insufficient resources, status=%08x\n"
    ...
    if (IS_WINDOWSXP_OR_LATER()) {
        ...
        lCompletionContext.RecordList = recordList;
        ...
        if (!SpyIsAttachedToDevice( vpb->DeviceObject,
            ...
            SPY_LOG_PRINT( SPYDEBUG_DISPLAY_ATTACHMENT_NAMES,
                           ("FileSpy!SpyFsControlMountVolume:             Mount volume failure for   %p \"%wZ\", already attached\n",
                            ((PFILESPY_DEVICE_EXTENSION)attachedDeviceObject->DeviceExtension)->NLExtHeader.AttachedToDeviceObject,
    ...
    if (shouldLog) {
        ...
        if (recordList) {
            ...
            if (!returnValue) {
                ...
                //
                //  Mark that the FastIO returned FALSE
                //
...
--*/
{
    ...
    switch (IoControlCode) {
        ...
        //
        //  List all the devices that we are currently monitoring
        //
...
Routine Description:
    ...
    It gets called by the KTM at the top level for preprepare/prepare/commit/rollback.

Arguments:

    EnlistmentObject - Pointer to the enlistment object handle.

    RmContext - Pointer to the RM context that user supplied in TmEnableTmCallbacks.

    TransactionContext -Pointer the Tx context that user supplied in TmCreateEnlistment.

    TransactionNotification - Transaction notification code.

    ArgumentLength - Number of arguments in the argument array.

    Argument - Pointer of the argument array.
...
--*/
{
    ...
    PFILESPY_TRANSACTION_CONTEXT txData = (PFILESPY_TRANSACTION_CONTEXT)TransactionContext;
    PFILESPY_DEVICE_EXTENSION DeviceExtension;
    ...
    ASSERT( NULL != gSpyDynamicFunctions.PrePrepareComplete );
    ASSERT( NULL != gSpyDynamicFunctions.PrepareComplete );
    ASSERT( NULL != gSpyDynamicFunctions.CommitComplete );
    ASSERT( NULL != gSpyDynamicFunctions.RollbackComplete );
    ...
    if (FlagOn( gFileSpyDebugLevel, SPYDEBUG_TRACE_TX_OPS )) {

        SpyDumpTransactionNotify( txData, TransactionNotification );
    }

    DeviceExtension = txData->DeviceObject->DeviceExtension;

    //
    //  Log the transaction notification.
    //

    SpyLogTransactionNotify( txData,
                             TransactionNotification );

    //
    //  Process the transaction notifications.
    //
    ...
    switch (TransactionNotification) {
        ...
        case TRANSACTION_NOTIFY_PREPREPARE:
            ...
            status = (gSpyDynamicFunctions.PrePrepareComplete)( EnlistmentObject,
                                                                TmVirtualClock );
            break;

        case TRANSACTION_NOTIFY_PREPARE:

            status = (gSpyDynamicFunctions.PrepareComplete)( EnlistmentObject,
                                                             TmVirtualClock );
            break;

        case TRANSACTION_NOTIFY_COMMIT:

            status = (gSpyDynamicFunctions.CommitComplete)( EnlistmentObject,
                                                            TmVirtualClock );
            break;

        case TRANSACTION_NOTIFY_ROLLBACK:

            status = (gSpyDynamicFunctions.RollbackComplete)( EnlistmentObject,
                                                              TmVirtualClock );
            break;

        default:

            ASSERT(!"Filespy!SpyKtmNotification: Unkown transaction notification value");
            break;
    }

    //
    //  Free the transaction context if in the commit or rollback phase.
    //
