...
GetIrpName (
    ...
    __in ULONG FsctlCode,
    __out_bcount(MajorCodeNameSize) PCHAR MajorCodeName,
    __in  ULONG MajorCodeNameSize,
    __out_bcount(MinorCodeNameSize) PCHAR MinorCodeName,
    __in  ULONG MinorCodeNameSize
    )
/*++
...
--*/
{
    ...
    switch (MajorCode) {
        ...
        case IRP_MJ_READ:
            ...
            switch (MinorCode) {
                ...
                case IRP_MN_COMPLETE_MDL_DPC:
                    ...
                    break;
                default:
                    StringCbPrintfA( nameBuf, sizeof(nameBuf), UnknownIrpMinor, MinorCode );
...
#endif
                        default:
                            StringCbPrintfA( nameBuf, sizeof(nameBuf), "Unknown FSCTL (%u)", MinorCode );
                            irpMinorString = nameBuf;
                            break;
                    }

                    StringCbPrintfA( nameBuf, sizeof(nameBuf), "%s (USER)", irpMinorString );
                    irpMinorString = nameBuf;
                    break;
        ...
        default:
            StringCbPrintfA( nameBuf, sizeof(nameBuf), "Unknown Irp major code (%u)", MajorCode );
            irpMajorString = nameBuf;
    }

    StringCbCopyA( MajorCodeName, MajorCodeNameSize, irpMajorString );
    StringCbCopyA( MinorCodeName, MinorCodeNameSize, irpMinorString );
}
