...
--*/
{
    ...
    if ( lstrlen( lpNetResource->lpLocalName ) > 1 )
    {
        if ( lpNetResource->lpLocalName[1] == L':' )
        {
            ...
            LocalName[1] = L':';
            LocalName[2] = L'\0';
            StringCchCopyW( ConnectionName, 128, DD_NULMRX_FS_DEVICE_NAME_U );
            StringCchCatW( ConnectionName, 128, L"\\;" );
            StringCchCatW( ConnectionName, 128, LocalName );
            ConnectionName[127] = 0;
        }
        else
        {
    ...
    if (Status == WN_SUCCESS)
    {
        ...
        {
            StringCchCatW( ConnectionName, 128, lpNetResource->lpRemoteName + 1 );
            ConnectionName[127] = 0;
    ...
    if ( lstrlen( lpName ) > 1 )
    {
        if ( lpName[1] == L':' )
        {
            ...
            if ( Status == WN_SUCCESS && CopyBytes > 0 && CopyBytes < 128 * sizeof(WCHAR) )
            {
                ...
                StringCchCatW( ConnectionName, 128, L"\\;" );
                StringCchCatW( ConnectionName, 128, LocalName );
                StringCchCatW( ConnectionName, 128, RemoteName );
            ...
            if ( ConnectionList[i] )
            {
                ...
                else
                {
                    ...
                    *StringZone++ = L'\0';
                    ...
                    StringCchCopyW( StringZone, sizeof(NULMRX_PROVIDER_NAME_U)/sizeof(WCHAR), NULMRX_PROVIDER_NAME_U );
...
ULONG _cdecl WideDbgPrint( __in LPTSTR Format, ... )
{   
    ULONG rc = 0;
    TCHAR szbuffer[256];
