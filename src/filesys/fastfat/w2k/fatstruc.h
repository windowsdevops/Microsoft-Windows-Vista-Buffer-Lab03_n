...
typedef struct _FAT_DATA {
    ...
    //
    //  A pointer to the filesystem device object we created.
    //

    PVOID FileSystemDeviceObject;
...
typedef struct _FAT_CLUSTERS_FREE_LOG_ENTRY {
    ULONG nClustersBefore;
    ULONG nClustersAfter;
    ULONG nLineNumber;
} FAT_CLUSTERS_FREE_LOG_ENTRY;

#define MAX_FAT_CLUSTERS_FREE_LOG (2000)
...
typedef struct _FAT_WINDOW {
    ...
    FAT_CLUSTERS_FREE_LOG_ENTRY ClustersFreeLog[MAX_FAT_CLUSTERS_FREE_LOG];
    ULONG iNextClustersFreeEntry;
    ULONG nNumberOfRollovers;
...
typedef struct _NON_PAGED_FCB {
    ...
    FAST_MUTEX FCBAdvancedHeaderMutex;
