...

{
    ...
    Results = FsRtlLookupLastLargeMcbEntryAndIndex( (PLARGE_MCB)Mcb,
                                                    &LiVbo,
                                                    &LiLbo,
                                                    &LocalIndex );
    ...
    )
{
    ...
    FsRtlRemoveLargeMcbEntry((PLARGE_MCB) Mcb,
                        (LONGLONG) Vbo,
                        (LONGLONG) SectorCount);
}
...

{
    ...
    try {
        ...
        //
        //  The OS/2 Boot Manager partition boot sector mimics a FAT16 partition
        //  but does not adhere to the FAT16 file system.  For example, the
        ...
        if (( NT_SUCCESS( StatusPartInfo )) &&
            ( PartitionInformation.PartitionType == PARTITION_OS2BOOTMGR ) &&
            ( Vcb->Bpb.Sectors != 0 ) &&
        ...
        if (IsBpbFat32(&BootSector->PackedBpb)) {
            ...
            //  its users are a bit silly, thinking that a BPB exists wholly in the first 0x24
            //  bytes.
            //
        ...
        //  with screwed up information by effectively disabling FsInfo updates.
        //
...

{
    ...
    try {
        ...
        if (!AbnormalTermination() || ExIsResourceAcquiredExclusive( &Vcb->Resource )) {
...

{
    ...
    ASSERT( ExIsResourceAcquiredExclusive( &Vcb->Resource ) &&
...

{
    ...
    try {
        ...
        //  a handle on it associated with this very request.
        //
...

{
    ...
    if ((TypeOfOpen != UserFileOpen
         ...
         //
         //  Fast minimal disable of directory defrag until stream FO use
         //  is fixed up.
         //

#ifdef FAT_DEFRAG_DIRECTORY
    ...
    if (NodeType(FcbOrDcb) == FAT_NTC_ROOT_DCB &&
        FileObject->SectionObjectPointer == NULL) {

        FileObject->SectionObjectPointer = &FcbOrDcb->NonPaged->SectionObjectPointers;
    }
...

{
    ...
    //  pass to our maxumum. We must also align the file offset to a
    ...
    )
{
    ...
    Buffer = FsRtlAllocatePool( NonPagedPoolCacheAligned, PAGE_SIZE );
