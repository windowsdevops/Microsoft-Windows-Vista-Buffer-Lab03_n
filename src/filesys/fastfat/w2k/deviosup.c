...
#if 0 // The corruption was happening on the SCSI bus. (1/11/93)
...

{
    ...
    //
    //  Note that it is convenient for us to be in the user process when
    //  this is done so that we have a chance of billing the right process
    //  for pool quota.  It is harmless if we are already in the system
    //  process ... which we clearly cannot help if some overlying filter
    //  has already posted the request.
    //
    ...
    if (Irp->AssociatedIrp.SystemBuffer == NULL) {
        ...
        Irp->AssociatedIrp.SystemBuffer = FsRtlAllocatePoolWithQuota( NonPagedPool, BufferLength );
