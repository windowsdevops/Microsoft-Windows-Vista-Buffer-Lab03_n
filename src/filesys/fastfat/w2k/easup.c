...

{
    ...
    try {
        ...
        if (Vcb->VirtualEaFile != NULL) {
            ...
            if (CreateFile) {
                ExAcquireResourceExclusive( Vcb->RootDcb->Header.Resource, TRUE );
            } else {
                ExAcquireResourceShared( Vcb->RootDcb->Header.Resource, TRUE );
            }
            ...
            //
...

                {
                    ...
                    (*EaDirent)->FirstClusterOfFile =
                            (FAT_ENTRY) FatGetIndexFromLbo( Vcb, FirstLboOfFile );
                }
