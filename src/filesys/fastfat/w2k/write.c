...

{
    ...
    if (IoGetCurrentIrpStackLocation(Irp)->DeviceObject != FatData.FileSystemDeviceObject) {
...

{
    ...
    ASSERT(!(NonCachedIo == FALSE && TypeOfWrite == VirtualVolumeFile));
    ...
    if (!PagingIo && !WriteToEof && (TypeOfWrite != UserVolumeOpen)) {
