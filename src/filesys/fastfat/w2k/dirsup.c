...

{
    ...
    if ( UnusedVbo + (DirentsNeeded * sizeof(DIRENT)) <=
        ...
        if (ByteOffset != -1) {
            ...
            if ((FatIsFat32(ParentDirectory->Vcb) &&
                 ParentDirectory->Header.AllocationSize.LowPart >= (64 * 1024 * sizeof(DIRENT))) ||
...

{
    ...
    ASSERT( ExIsResourceAcquiredExclusive( &FcbOrDcb->Vcb->Resource ));
    ...
    try {
        ...
        ExAcquireResourceExclusive( FcbOrDcb->ParentDcb->Header.Resource, TRUE );
        ...
        ExReleaseResource( FcbOrDcb->ParentDcb->Header.Resource );
    }
...

{
    ...
    ASSERT( ExIsResourceAcquiredShared( ParentDirectory->Header.Resource ) ||
            ExIsResourceAcquiredExclusive( ParentDirectory->Header.Resource ) ||
            ExIsResourceAcquiredShared( &ParentDirectory->Vcb->Resource ) ||
            ExIsResourceAcquiredExclusive( &ParentDirectory->Vcb->Resource ));
