...
__SmbPseDbgCheckOEMdls(
    __in PSMB_PSE_ORDINARY_EXCHANGE OrdinaryExchange,
    __in PCSTR MsgPrefix,
    __in PCSTR File,
    __in ULONG Line
    )
{
...
VOID
SmbPseFinalizeOETrace(
    __in PCSTR text,
    __in ULONG finalstate
    )
{
    PAGED_CODE();
        ...
        case SMB_COM_READ_ANDX:
            {
                ...
                if (!FlagOn(ReceiveModelParamsFlags,SMBPSE_RMP_NOCOPY_HANDLER)) {
                    ...
                    NoCopyReceiveHandler = (PSMBPSE_NOCOPY_RECEIVE_HANDLER) 
                                           SmbPseReceiveHandlers[ReceiveModelParams->ReceiveHandlerToken];
