...
--*/
{
    ...
    ReceiveParamsBuffer = (PBYTE)&CreateResponse;
    ReceiveParamsBufferLength = sizeof(RESP_CREATE_WITH_SD_OR_EA);
    ...
    if ((EaLength==0)||(SdLength==0)) {
        ...
        ULONG CombinedBufferLength;
         
        __assume_bound( EaOffset );
        __assume_bound( EaLength );

        CombinedBufferLength = EaOffset + EaLength;
