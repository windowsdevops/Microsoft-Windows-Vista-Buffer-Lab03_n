...
ULONG_PTR RdrInstallCheck( void )
{
    ...
    TCHAR tszWindowsPath[_MAX_PATH];
    ULONG_PTR teststep;
    ULONG len = 0;
    ...
    for ( teststep = installcheck_start; teststep < installcheck_done; teststep++ )
    {
        switch ( teststep )
        {
            ...
            {
                len = GetWindowsDirectory( tszWindowsPath, _MAX_PATH );
            ...
            {
                StringCchCopy( tszTestPath, sizeof(tszTestPath), tszWindowsPath );
                StringCchCat( tszTestPath, sizeof(tszTestPath) , DRIVER_FILE_PATH );
            ...
            {
                StringCchCopy( tszTestPath, sizeof(tszTestPath), tszWindowsPath );
                StringCchCat( tszTestPath, sizeof(tszTestPath), PROVIDER_FILE_PATH );
                th = CreateFile( tszTestPath, 0, 0, NULL, OPEN_EXISTING, 0, NULL );
                if ( th == INVALID_HANDLE_VALUE )
                {
                    tc = SETUP_MISSING_FILE;
...
--*/
{
    ...
    {
        DbgP(( TEXT("Issuing Rdr Start Ioctl\n") ));
...
{
    {
        TEXT("Devicename"),
        REG_SZ,
        sizeof(DD_SMBMRX_FS_DEVICE_NAME),
        DD_SMBMRX_FS_DEVICE_NAME 
    },
    {
        TEXT("ProviderPath"),
        REG_EXPAND_SZ,
...
BOOL RdrSetupProviderOrder( void )
{
    ...
    if ( len > 0 && pOrderString )
    {
        if ( !RdrFindProviderInOrder( pOrderString, PROVIDER_NAME ) )
        {
            ...
            if ( pNewOrderString )
            {
                StringCbCopy( pNewOrderString, len, pOrderString );
                StringCbCat( pNewOrderString, len, TEXT(",") );
                StringCbCat( pNewOrderString, len, PROVIDER_NAME );
...
{   
    ...
    rc = StringCchVPrintfW( szbuffer, 256, Format, marker );
