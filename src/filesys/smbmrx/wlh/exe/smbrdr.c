...
int WINAPI WinMain(
  ...
  __in LPSTR lpCmdLine,      // pointer to command line
  __in int nCmdShow          // show state of window
);
    ...
    if ( GetLastError( ) != ERROR_ALREADY_EXISTS )
    {
        ...
        if ( RdrIsInstalled )
        {
            ...
            if ( hDlg )
            {
                ...
                StringCchCopy( nid.szTip, 128, TASK_TIP );
...
//Leave();
    ...
    case WM_TIMER:
    {
        ...
        if ( wParam == TIMER_ID && pdds->RdrState > RDR_STOPPED )
        {
            ...
            StringCchPrintf( timestring, 10, TEXT("%02d:%02d"), pdds->ElapsedStartTime / 60,
