...
ULONG _cdecl DbgPrint( __in LPTSTR Format, ... );
...
ULONG FillInEaBuffer( 
    __in_opt LPTSTR pUserName, 
    __in_opt LPTSTR pPassword, 
    __out_bcount(ByteCount) PBYTE pEaData, 
    LONG ByteCount )
{
    ...
    {
        if (ByteCount < sizeof(FILE_FULL_EA_INFORMATION)) {
            return 0;
        }
    ...
    if ( pUserName )
    {
        ByteCount -= ( sizeof(FILE_FULL_EA_INFORMATION) + sizeof("UserName") );
        if (ByteCount <= 0) {
            return 0;
        }
        ...
        ByteCount -= (thisEa->EaValueLength + sizeof(LONG));
        if (ByteCount <= 0) {
            return 0;
        }
    ...
    if ( pPassword )
    {
        ...
        ByteCount -= ( sizeof(FILE_FULL_EA_INFORMATION) + sizeof("Password") );
        if (ByteCount <= 0) {
            return 0;
        }
    ...
    if ( pDomain )
    {
        ...
        ByteCount -= ( sizeof(FILE_FULL_EA_INFORMATION) + sizeof("Domain") );
        if (ByteCount <= 0) {
            return 0;
        }
        ...
        ByteCount -= (thisEa->EaValueLength);
        if (ByteCount <= 0) {
            return 0;
        }
...
--*/
{
    ...
    ConnectionName.Length = ConnectionName.MaximumLength - sizeof(WCHAR);
    ...
    // Copy the name into the buffer. Buffer is zero initialized on allocation.

    CopyMemory( ConnectionName.Buffer,
                SmbMRxDeviceName.Buffer,
                SmbMRxDeviceName.Length);

    StringCbCatW( ConnectionName.Buffer, ConnectionName.MaximumLength, L"\\;" );
    if (pLocalName != NULL)
    {
        StringCbCatW( ConnectionName.Buffer, ConnectionName.MaximumLength, pLocalName );
    }

    StringCbCatW( ConnectionName.Buffer, ConnectionName.MaximumLength, pRemoteName );
    ...
    if ( ConnectInfo )
    {
        ...
        ConnectInfo->EaDataLength = FillInEaBuffer( pUserName,
                                                    pPassword,
                                                    (PBYTE) ConnectInfo->InfoArea + ConnectInfo->EaDataOffset, 
                                                    MAX_CONNECT_INFO_SIZE - FIELD_OFFSET( SMBMRX_CONNECTINFO, InfoArea ) - ConnectInfo->EaDataOffset );
        TransferBytes = 0;
    ...
    if (Status == WN_SUCCESS)
    {
        ...
        if (Status == WN_SUCCESS)
        {
            ...
            if (FreeEntryFound)
            {
                ...
                StringCchCopyW( pSmbMrxNetResource->UserName, MAX_PATH, pUserName );
                
                if ( *pPassword )
                {
                    StringCchCopyW( pSmbMrxNetResource->Password, MAX_PATH, pPassword);
                }
                else
                {
...
NPGetConnection(
    ...
    __out_ecount_opt(*lpBufferSize) LPWSTR lpRemoteName,
...
--*/
{
    ...
    StringCchCopyW( LocalPath, MAX_PATH, lpLocalPath );
    ...
    StringCchCopyW( UniversalName, MAX_PATH, RemoteName );

    if (pRemainingPath)
    {
        StringCchCatW( UniversalName, MAX_PATH, pRemainingPath );
    }
