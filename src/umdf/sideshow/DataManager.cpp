...
HRESULT CDataManager::AddApplication(GUID guidApplicationId,
                                     __in_ecount(SizeOfTitle) wchar_t* wszTitle,
                                     size_t SizeOfTitle,
                                     HICON* pIcon,
                                     size_t SizeOfIcon)
{
    ...
    {
        CNodeApplication* pNewApplicationNode = new CNodeApplication(guidApplicationId,
                                                                     wszTitle,
                                                                     SizeOfTitle,
                                                                     pIcon,
                                                                     SizeOfIcon);
        if (NULL != pNewApplicationNode)
        {
            m_pApplicationNodes.push_back(pNewApplicationNode);
        }
        else
        {
            hr = E_OUTOFMEMORY;
...
HRESULT CDataManager::UpdateApplicationNode(GUID guidApplicationId,
                                            __in_ecount(SizeOfBody) wchar_t* wszBody,
                                            size_t SizeOfBody)
{
    EnterCriticalSection(&m_CriticalSection);
...
HRESULT CDataManager::DeleteApplicationContent(GUID guidApplicationId)
{
    ...
    HRESULT hrNotifications = DeleteAllNotificationsForAnApplication(guidApplicationId);
...
HRESULT CDataManager::ChangeSortOrder(__in_ecount(cApps) const APPLICATION_ID* pApps, const DWORD cApps)
{
    ...
    for (DWORD nApp = 0; nApp < cApps; ++nApp)
    {
        ...
        if (QuantityOfApplicationNodes == index)
        {
            pNewNode = new CNodeApplication(pApps[nApp],
                                            NULL,
                                            0,
                                            NULL,
                                            0);
        ...
        if (NULL != pNewNode)
        {
            m_pApplicationNodes.push_back(pNewNode);
        }
    }
                                      ...
                                      FILETIME notificationExpirationTime)
{
    ...
    CNodeNotification* pNewNotificationNode = new CNodeNotification(guidApplicationId,
                                                                    ulNotificationId,
                                                                    wszTitle,
                                                                    SizeOfTitle,
                                                                    wszBody,
    ...
    if (NULL != pNewNotificationNode)
    {
        m_pNotificationNodes.push_back(pNewNotificationNode);
    }
    else
    {
        hr = E_OUTOFMEMORY;
...
HRESULT CDataManager::DeleteAllNotificationsForAnApplication(GUID guidApplicationId)
{
