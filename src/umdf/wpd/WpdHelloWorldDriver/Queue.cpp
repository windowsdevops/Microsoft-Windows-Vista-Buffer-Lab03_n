    ...
    DWORD*      pdwBytesWritten)
{
    ...
    if (hr == S_OK)
    {
        hr = pWpdBaseDriver->DispatchWpdMessage(pParams, m_pResults);
    ...
    // Clear IPortableDeviceValues so that we can reuse the collection.
    if (hr == S_OK)
    {
        hr = m_pResults->Clear();
        CHECK_HR(hr, "Failed to clear pResults");
    }
    ...
    )
{
    ...
    // Create the reusable WPD results collection
    if ((hr == S_OK) &&
        (m_pResults == NULL))
    {
        ...
        )
{
    ...
    if(IS_WPD_IOCTL(ControlCode))
    {
        ...
        // Mark this request as cancellable
        pRequest->MarkCancelable(static_cast<IRequestCallbackCancel*>(this));
