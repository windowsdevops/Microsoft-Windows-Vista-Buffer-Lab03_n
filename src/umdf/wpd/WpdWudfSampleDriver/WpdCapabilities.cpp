...
const ContentTypeFormatPair g_ContentTypeFormatPairs[] =
{
    ...
    {&WPD_CONTENT_TYPE_IMAGE, &WPD_OBJECT_FORMAT_JFIF},
...
HRESULT WpdCapabilities::Initialize(FakeDevice *pFakeDevice)
{
    ...
    IPortableDeviceValues*  pResults)
{
    ...
    if (hr == S_OK)
    {
        ...
        if (hr == S_OK)
        {
            if (IsEqualPropertyKey(WPD_COMMAND_OBJECT_RESOURCES_SEEK, Command))
            {
