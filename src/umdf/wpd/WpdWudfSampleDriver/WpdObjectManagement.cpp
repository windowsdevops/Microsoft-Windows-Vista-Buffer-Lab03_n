...
HRESULT WpdObjectManagement::Initialize(FakeDevice *pFakeDevice)
{
    ...
    IPortableDeviceValues*  pResults)
{
    ...
    if (hr == S_OK)
    {
        ...
        hr = CreateObjectManagementContext((ContextMap*) pContextMap, pszObjectName, pValues, &pszContext);
 ...
 *  - Return the new ObjectID in WPD_PROPERTY_OBJECT_MANAGEMENT_OBJECT_ID.
 */
    ...
    IPortableDeviceValues*  pResults)
{
    ...
    // Get the resource context for this creation request
    if (hr == S_OK)
    {
    ...
    if (hr == S_OK)
    {
        hr = m_pFakeDevice->SaveNewObject(pContext->ObjectProperties, &pszObjectID);
        CHECK_HR(hr, "Failed to save new object [%ws] to device", pContext->Name);
    }
    // We're done with the context
    SAFE_RELEASE(pContext);

    // Destroy the context associated with this creation request, since it is no longer needed
    if (hr == S_OK)
    {
        hr = DestroyObjectManagementContext(pContextMap, pszContext);
