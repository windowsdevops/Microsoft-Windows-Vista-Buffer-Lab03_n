...
public:
    ...
    virtual HRESULT GetSupportedProperties(IPortableDeviceKeyCollection **ppKeys)
    {
        ...
        if (hr == S_OK)
        {
            hr = AddSupportedProperties(WPD_OBJECT_FORMAT_JFIF, ppKeys);
            CHECK_HR(hr, "Failed to add additional properties for WPD_OBJECT_FORMAT_JFIF");
        }
        ...
        IPortableDeviceValues** ppAttributes)
    {
        ...
        if ((hr == S_OK) && (IsEqualPropertyKey(Key, WPD_RESOURCE_THUMBNAIL)))
        {
            ...
            if (hr == S_OK)
            {
                hr = pAttributes->SetGuidValue(WPD_RESOURCE_ATTRIBUTE_FORMAT, WPD_OBJECT_FORMAT_JFIF);
                CHECK_HR(hr, "Failed to set WPD_RESOURCE_ATTRIBUTE_FORMAT");
            }
        }

        if ((hr == S_OK) && (IsEqualPropertyKey(Key, WPD_RESOURCE_AUDIO_CLIP)))
        {
