...
VOID
CMyDevice::ClearTargets(
    )
{
    SAFE_RELEASE(m_pIUsbInputPipe);
    SAFE_RELEASE(m_pIUsbOutputPipe);
    SAFE_RELEASE(m_pIUsbInterruptPipe);
    SAFE_RELEASE(m_pIUsbInterface);
    SAFE_RELEASE(m_pIUsbTargetDevice);    
}

CMyDevice::~CMyDevice(
    )
{
    SAFE_RELEASE(m_SwitchChangeQueue);
}
...
--*/
{
    ...
    if (SUCCEEDED(hr)) 
    {
        ...
        m_pIUsbInputPipe = pIUsbInputPipe;
        m_pIUsbOutputPipe = pIUsbOutputPipe;
        m_pIUsbInterruptPipe = pIUsbInterruptPipe;
    }
    else
    {
        SAFE_RELEASE(pIUsbTargetDevice);
        SAFE_RELEASE(pIUsbInterface);
        SAFE_RELEASE(pIUsbInputPipe);
        SAFE_RELEASE(pIUsbOutputPipe);
        SAFE_RELEASE(pIUsbInterruptPipe);
    }
...
/*++
  ...
  Routine Description:
    ...
    This method is invoked when the device enters the D0 state.  In response
    it starts a ping-pong read of the interrupt pipe to check the switch state.
    ...
    )
{
    ...
    // In the surprise remove scenario, the OnSurpriseRemoval callback gets 
    // called before the OnD0Exit callback. The OnSurpriseRemoval callback
    // would have removed the IO target. Hence it does not make sense to stop 
    // the target that's already been removed. The check below will ensure that
    // we only stop targets that are started.
    //
...
{
    RemoveTarget(m_pIUsbInterruptPipe,
                 TRUE); //bSurpriseRemove
    
    return;
}

VOID
CMyDevice::StartTarget(
    ...
    )
{
    ...
    pStateMgmt->Stop(WdfIoTargetCancelSentIo);
...
VOID
CMyDevice::RemoveTarget(
    IWDFIoTarget * pTarget,
    BOOL bSurpriseRemove
    )
{
    IWDFIoTargetStateManagement * pStateMgmt = NULL;
    ...
    pStateMgmt->Remove(bSurpriseRemove);
