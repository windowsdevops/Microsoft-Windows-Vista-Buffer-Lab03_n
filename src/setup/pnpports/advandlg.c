    ...
    )
{
    ...
    LoadString(g_hInst, IDS_ADVANCED_SETTINGS_FOR, szFormat, ARRAYSIZE(szFormat));
    StringCchPrintf(szBuffer, ARRAYSIZE(szBuffer), szFormat, advancedData->szComName);
                          ...
                          )
{
    ...
    if (GetDlgCtrlID(hTrackbar) == IDC_RECEIVE_SLIDER) {
        ...
        //
        StringCchPrintf(szCurrentValue, ARRAYSIZE(szCurrentValue), TEXT("(%d)"), RxValues[position-1]);
        ctrlID = IDC_RXVALUE;
    }
    else {
        //
        ...
        //
        StringCchPrintf(szCurrentValue, ARRAYSIZE(szCurrentValue), TEXT("(%d)"), position);
        ctrlID = IDC_TXVALUE;
    }
    ...
    )
{
    ...
    {
        StringCchCopy(txt, ARRAYSIZE(txt), szTemp);
        StringCchPrintf(szTemp, ARRAYSIZE(szTemp), _T("%d)"), Value);
        StringCchCat(txt, ARRAYSIZE(txt), szTemp);
    }
    else 
    {
        StringCchCopy(txt, ARRAYSIZE(txt), _T("Low"));
    }
    ...
    )
{
    ...
    if (!AdvancedData->UseRxFIFOControl ||
        ...
        StringCchPrintf(szCurrentValue, ARRAYSIZE(szCurrentValue), TEXT("(%d)"), AdvancedData->RxFIFO);
        ...
        StringCchPrintf(szCurrentValue, ARRAYSIZE(szCurrentValue), TEXT("(%d)"), AdvancedData->TxFIFO);
    ...
    if (!AdvancedData->HidePolling) {
        ...
        if (periodIdx == NUM_POLLING_PERIODS) {
            StringCchPrintf(szCurrentValue,
                     ARRAYSIZE(szCurrentValue),
    ...
    )
{
    ...
    StringCchPrintf(szCurrentValue, ARRAYSIZE(szCurrentValue), TEXT("(%d)"), AdvancedData->FifoRxMax);
    SetDlgItemText(ParentHwnd, IDC_RXVALUE, szCurrentValue);

    Trackbar_SetPos(GetDlgItem(ParentHwnd, IDC_TRANSMIT_SLIDER), TRUE, AdvancedData->FifoTxMax);
    StringCchPrintf(szCurrentValue, ARRAYSIZE(szCurrentValue), TEXT("(%d)"), AdvancedData->FifoTxMax);
    SetDlgItemText(ParentHwnd, IDC_TXVALUE, szCurrentValue);
    ...
    )
{
    ...
    for(nEntries = j = 0, i = MIN_COM-1; i < MAX_COM_PORT; i++) {
        ...
        StringCchPrintf(szCom, ARRAYSIZE(szCom), TEXT("COM%d"), i+1);
        if (*current & mask) {
            StringCchCat(szCom, ARRAYSIZE(szCom), szInUse);
        }
    ...
    )
{
   ...
   if (dwError != ERROR_SUCCESS) {
      StringCchPrintf(AdvancedData->szComName, ARRAYSIZE(AdvancedData->szComName), TEXT("COMX"));
   }
...
{
    PTSTR pszOldTitle = NULL;
    PTSTR pszOldLocation = NULL;
    PTSTR pszNewTitle = NULL;
    DWORD cchOldTitle;
    DWORD cchNewComName;
    DWORD cchOldComName;
    DWORD cchNewTitle;
    ...
    cchOldTitle = GetWindowTextLength(Hwnd);
    if (cchOldTitle == 0) {
        return;
    }
    ...
    cchOldTitle++;
    pszOldTitle = (PTSTR) LocalAlloc(LPTR, cchOldTitle * sizeof(TCHAR));
    if (!pszOldTitle) {
        return;
    }

    if (!GetWindowText(Hwnd, pszOldTitle, cchOldTitle)) {
        goto exit;
    }

    pszOldLocation = wcsstr(pszOldTitle, OldComName);
    if (!pszOldLocation) {
        goto exit;
    }

    cchNewComName = lstrlen(NewComName);
    cchOldComName = lstrlen(OldComName);
    cchNewTitle = cchOldTitle;
    if (cchNewComName > cchOldComName) {
        offset = cchNewComName - cchOldComName;
        cchNewTitle += offset;
    }
    
    pszNewTitle = (PTSTR) LocalAlloc(LPTR, cchNewTitle * sizeof(TCHAR));
    if (!pszNewTitle) {
        goto exit;
    }
    ...
    //
    offset = (DWORD)(pszOldLocation - pszOldTitle);
    StringCchCopyN(pszNewTitle, cchNewTitle, pszOldTitle, offset + 1);         // 1
    StringCchCopy(pszNewTitle + offset, cchNewTitle - offset, NewComName);     // 2
    StringCchCopy(pszNewTitle + (offset + cchNewComName), cchNewTitle - (offset + cchNewComName), pszOldLocation + cchOldComName);      // 3

    SetWindowText(Hwnd, pszNewTitle);

exit:
    LocalFree(pszOldTitle);
    LocalFree(pszNewTitle);
}
    ...
    )
{
    ...
    cchOldName = (oldNameLen + 2);
    oldName = (PTCHAR) LocalAlloc(LPTR, cchOldName * sizeof(TCHAR));

    // if we allocated memory, copy string, other wise return without doing anything
    if(oldName)
    {
        StringCchCopy(oldName, cchOldName, OldComName);
        StringCchCat(oldName, cchOldName, m_szColon);
    }
    else
    {
        return;
    ...
    // allocate memory for string (adjusted for colon and NULL terminator)
    cchNewName = newNameLen + 2;
    newName = (PTCHAR) LocalAlloc(LPTR, cchNewName * sizeof(TCHAR));

    // if we allocated memory, copy string, other wise return without doing anything
    if(newName)
    {
        StringCchCopy(newName, cchNewName, NewComName);
        StringCchCat(newName, cchNewName, m_szColon);
    }
    else
    {
        if(oldName)
    ...
    IN UINT             NewComNum)
{
    ...
    if (!QueryDosDevice(AdvancedData->szComName, buffer, BUFFER_SIZE-1)) {
        ...
        StringCchCopy(szComFileName, ARRAYSIZE(szComFileName), L"\\\\.\\");
        StringCchCat(szComFileName, ARRAYSIZE(szComFileName), AdvancedData->szComName);
    ...
    StringCchPrintf(szNewComName, ARRAYSIZE(AdvancedData->szNewComName), _T("COM%d"), NewComNum);
    ...
    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE,
        ...
        i = 0;
        do {
            dwSerialSize = ARRAYSIZE(szSerial);
    ...
    if (LoadString(g_hInst,
                   ...
                   ARRAYSIZE(szFriendlyNameFormat)) &&
        SetupDiGetDeviceRegistryProperty(AdvancedData->DeviceInfoSet,
                                         ...
                                         sizeof(szDeviceDesc),
                                         NULL)) {
        StringCchPrintf(buffer, ARRAYSIZE(buffer), szFriendlyNameFormat, szDeviceDesc, szNewComName);
    }
    else {
        //
        // Use the COM port name straight out
        //
        StringCchCopy(buffer, ARRAYSIZE(buffer), szNewComName);
    }
