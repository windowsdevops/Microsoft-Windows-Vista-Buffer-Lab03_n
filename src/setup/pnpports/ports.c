...
#error MAX_DEVICE_ID_LEN is greater than MAX_PATH.  Update charBuffer.
    ...
    //
    StringCchPrintf(szPortName, ARRAYSIZE(szPortName), TEXT("%s%d"), m_szCOM, comPortNumber);
    ...
    if(!SetupDiInstallDevice(DeviceInfoSet, DeviceInfoData)) {
        if (hComDB != HCOMDB_INVALID_HANDLE_VALUE) {
            if ( bPortClaimed ) {
                ComDBReleasePort(hComDB,comPortNumber);
            }
            ComDBClose(hComDB);
        }
        return GetLastError();
    ...
    if (LoadString(g_hInst,
        ...
        SetupDiGetDeviceRegistryProperty(DeviceInfoSet,
                                         ...
                                         sizeof(deviceDesc),
                                         NULL)) {
        StringCchPrintf(charBuffer, ARRAYSIZE(charBuffer), friendlyNameFormat, deviceDesc, szPortName);
    }
    else {
        StringCchCopy(charBuffer, ARRAYSIZE(charBuffer), szPortName);
    }
    ...
    // don't already exist.
    //
    StringCchCat(szPortName, ARRAYSIZE(szPortName), m_szColon);
    charBuffer[0] = TEXT('\0');
...
void InitStrings(void)
{
    ...
    LoadString(g_hInst,
               INITS,
               g_szErrMem,
               ARRAYSIZE(g_szErrMem));
    LoadString(g_hInst,
               IDS_INIT_NAME,
               g_szPortsApplet,
               ARRAYSIZE(g_szPortsApplet));
