...
//
HRESULT WiaMCamOpen(MCAM_DEVICE_INFO *pDeviceInfo, __in PWSTR pwszPortName)
{
    DBG_FN("WiaMCamOpen");
...
HRESULT WiaMCamGetItemInfo(MCAM_DEVICE_INFO *pDeviceInfo, MCAM_ITEM_INFO *pItemInfo)
{
    ...
    //
    // If pItemInfo is null it means that IWiaItem::CreateChildItem has
    // been called for a new child item which is not supported
    //
    if (!pItemInfo)
    {
        hr = E_NOTIMPL;
...
//
HRESULT FakeCamOpen(__in PTSTR ptszPortName, MCAM_DEVICE_INFO *pDeviceInfo)
{
    DBG_FN("FakeCamOpen");
    ...
    {
        StringCchCopy(ptszRootPath, uiRootPathSize, ptszPortName);
    }
...
// items.
//
HRESULT SearchDir(FAKECAM_DEVICE_INFO *pPrivateDeviceInfo, MCAM_ITEM_INFO *pParent, __in PTSTR ptszPath)
{
    DBG_FN("SearchDir");
    ...
    //
    if (FAILED(StringCchPrintf(tszTempStr, cchTempStrSize, _T("%s\\*"), ptszPath)))
    {
    ...
    // Make sure search path fits into the buffer and gets zero-terminated
    //
    if (FAILED(StringCchPrintf(tszTempStr, cchTempStrSize, _T("%s\\*.jpg"), ptszPath)))
    {
        wiauDbgError("SearchDir", "Too long path for search");
        hr = E_FAIL;
        goto Cleanup;
...
//
HRESULT SearchForAttachments(FAKECAM_DEVICE_INFO *pPrivateDeviceInfo, MCAM_ITEM_INFO *pParent, __in PTSTR ptszMainItem)
{
    ...
    //
    StringCchCopy(tszTempStr, sizeof(tszTempStr) / sizeof(tszTempStr[0]) - 1, ptszMainItem);
    ...
    while (hr == S_OK)
    {
            ...
            !(_tcsstr(ptszMainItem, FindData.cFileName)))
        {
            ...
            //
            StringCchCopy(tszTempStr, sizeof(tszTempStr) / sizeof(tszTempStr[0]), ptszMainItem);
            ptcSlash = _tcsrchr(tszTempStr, TEXT('\\'));
            if (ptcSlash)
            {
                *ptcSlash = TEXT('\0');
...
HRESULT CreateFolder(FAKECAM_DEVICE_INFO *pPrivateDeviceInfo, MCAM_ITEM_INFO *pParent,
                     WIN32_FIND_DATA *pFindData, MCAM_ITEM_INFO **ppFolder, __in PTSTR ptszFullName)
{
    DBG_FN("CreateFolder");
    ...
    *ppFolder = NULL;
    ...
    //
    StringCchCopy(tszTempStr, sizeof(tszTempStr) / sizeof(tszTempStr[0]), pFindData->cFileName);
    ptcDot = _tcsrchr(tszTempStr, TEXT('.'));
    if (ptcDot)
        *ptcDot = TEXT('\0');
...
HRESULT CreateImage(FAKECAM_DEVICE_INFO *pPrivateDeviceInfo, MCAM_ITEM_INFO *pParent,
                    WIN32_FIND_DATA *pFindData, MCAM_ITEM_INFO **ppImage, __in PTSTR ptszFullName)
{
    DBG_FN("CreateImage");
...
HRESULT CreateNonImage(FAKECAM_DEVICE_INFO *pPrivateDeviceInfo, MCAM_ITEM_INFO *pParent,
                       WIN32_FIND_DATA *pFindData, MCAM_ITEM_INFO **ppNonImage, __in PTSTR ptszFullName)
{
...
HRESULT SetCommonFields(MCAM_ITEM_INFO *pItem,
                        __in PTSTR ptszShortName,
                        __in PTSTR ptszFullName,
                        WIN32_FIND_DATA *pFindData)
{
    ...
    StringCchCopy(ptszTempStr, iSize, ptszFullName);
...
//
HRESULT ReadDimFromJpeg(__in PTSTR ptszFullName, WORD *pWidth, WORD *pHeight)
{
    ...
    *pHeight = 0;
    ...
    {
        hr = E_FAIL;
        REQUIRE_SUCCESS(hr, "ReadDimFromJpeg", "Not a JFIF format image");
    }
...
// Exif files.
//
HRESULT ReadJpegHdr(__in PTSTR ptszFullName, BYTE **ppBuf)
{
    DBG_FN("ReadJpegHdr");
