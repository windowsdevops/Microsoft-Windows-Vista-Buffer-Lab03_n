...
STDMETHODIMP CShellExt::GetCommandString (UINT_PTR idCmd,
                                          UINT uType,
                                          UINT* pwReserved,
                                          __out_awcount((uType != GCS_VALIDATEW) && (uType != GCS_VERBW), cchMax) __typefix(LPWSTR) LPSTR pszName,
                                          UINT cchMax)
{
    ...
    switch (uType)
    {
        case GCS_VALIDATEA:
            ...
            if (pszName)
            {
                StringCchCopyA(pszName, cchMax, g_PathVerbA);
            }
            return S_OK;
        ...
        case GCS_VALIDATEW:
            ...
            if (pszName)
            {
                StringCchCopyW(reinterpret_cast<LPWSTR>(pszName), cchMax, g_PathVerbW);
            }
            return S_OK;
...
HRESULT GetSetRootPath (IWiaItem *pCamera, __out_ecount(cchMax) LPTSTR pszPath, int cchMax, BOOL bSet)
{
