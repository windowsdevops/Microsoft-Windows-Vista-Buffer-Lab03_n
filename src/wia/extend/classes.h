    ...
    public IShellExtInit, public IContextMenu, public IShellPropSheetExt
{
    ...
    END_COM_MAP()
        ...
        STDMETHODIMP GetCommandString (UINT_PTR idCmd, UINT uType,UINT* pwReserved,__out_awcount((uType != GCS_VALIDATEW) && (uType != GCS_VERBW), cchMax) __typefix(LPWSTR) LPSTR pszName,UINT cchMax);
        ~CShellExt ();
        CShellExt ();
};
