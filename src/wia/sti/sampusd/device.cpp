                                        ...
                                        HKEY hParametersKey )
{
    ...
    *szDeviceNameW = L'\0';
    ...
    if ( StrCmpI("AUTO", m_pszDeviceNameA) ){
        ...
        ActualLength = GetTempPathW(MAX_PATH,TempFile);
        ...
        if (SUCCEEDED(hres))
        {
            SHCreateDirectory(NULL,TempFile);

            if (!GetTempFileNameW(TempFile, L"StiTestDrvFile", 0, TempFile))
            {
        ...
        if (SUCCEEDED(hres))
        {
            m_DeviceDataHandle = CreateFileW( TempFile,
                                            GENERIC_READ | GENERIC_WRITE ,                     // Access mask
                                            0,    
                                            NULL,                              // SA
                                            CREATE_ALWAYS,                     // Create disposition
