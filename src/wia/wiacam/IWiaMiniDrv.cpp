...
HRESULT CWiaCameraDevice::drvGetDeviceErrorStr(
    LONG                        lFlags,
    LONG                        lDevErrVal,
    LPOLESTR                    __deref_out_opt *ppszDevErrStr,
    LONG                        *plDevErr)
{
...
HRESULT CWiaCameraDevice::ConstructFullName(MCAM_ITEM_INFO *pItemInfo, 
                                            __in_ecount(cchFullNameSize) WCHAR *pwszFullName, 
                                            INT cchFullNameSize)
{
    ...
    else 
    {
        ...
        {
            StringCchCopyW(pwszFullName, cchFullNameSize, m_bstrRootFullItemName);
        }
        else
        {
            hr = E_FAIL;
    ...
    if (SUCCEEDED(hr) && pItemInfo->pwszName) 
    {
        ...
        {
            StringCchCatW(pwszFullName, cchFullNameSize, L"\\");
            StringCchCatW(pwszFullName, cchFullNameSize, pItemInfo->pwszName);
        }
        else
        {
...
*
\**************************************************************************/
HRESULT CWiaCameraDevice::GetOLESTRResourceString(LONG lResourceID, __deref_out LPOLESTR *ppsz)
{
    ...
    *ppsz = (LPOLESTR)CoTaskMemAlloc(sizeof(wszStringValue));
    if (*ppsz != NULL) 
    {
        StringCchCopyW(*ppsz, sizeof(wszStringValue)/sizeof(WCHAR),wszStringValue);
    } 
    else 
    {
        hr = E_OUTOFMEMORY;
