...
DWORD gPageOrderPlayBackForDriver[][MAX_NUP_OPTIONS][MAX_NUP] = 
    {
        { //2-up
            //Portrait
...
BOOL GdiGetDevmodeForPagePvt(
    ...
    __in      DWORD               dwPageNumber,
    __out     PDEVMODEW           *ppCurrDM,
    __out_opt PDEVMODEW           *ppLastDM
  );
...
GetStartPageListForwardForDriver(
    ...
    __deref_out PPAGE_NUMBER          *ppMemoryHead,
    __deref_out PPAGE_NUMBER          *ppPageListHead,
    __in        DWORD                  dwSmallestPageOnSheet
    );


PDWORD
GetPlaybackPageOrderForDriverNup (
    __in DWORD                  dwNumberOfPagesPerSide,
    __in ENupDirection          eNupDirection,
    __in INT                    dmOrientation
    )
{
...
GetPageCoordinatesForNUp(
    __in      HDC                    hPrinterDC,
    __in      PEMF_ATTRIBUTE_INFO    pEMFAttr,
    __out     RECT                  *rectDocument,
    __out_opt RECT                  *rectBorder,
    __in      UINT                   uCurrentPageNumber,
    __out     LPBOOL                 pbRotate
    )
...

{
    ...
    lXPhyPage = pEMFAttr->lXPhyPage * yResolution;
    lYPhyPage = pEMFAttr->lYPhyPage * xResolution;
    ...
    *pbRotate = FALSE;
    ...
    else
    {
        ...
        //
        if (!BGetPageCoordinatesForNUpFromArray(pEMFAttr, &URect, (lXPrintPage > lYPrintPage)))
        {
            return FALSE;
        }
    }
...
SetDrvCopies(
    __in    HDC          hPrinterDC,
    __inout LPDEVMODEW   pDevmode,
    __in    DWORD        dwNumberOfCopies)
...
BOOL
DifferentDevmodes(
    __in_opt LPDEVMODE    pDevmode1,
    __in_opt LPDEVMODE    pDevmode2
    )
...
ResetDCForNewDevmode(
    ...
    __in            PEMF_ATTRIBUTE_INFO   pEMFAttr,
    __in            DWORD                 dwPageNumber,
    __in            BOOL                  bInsidePage,
    __in            DWORD                 dwOptimization,
    __out           LPBOOL                pbNewDevmode,
    __in            LPDEVMODE             pDevmode,
    __deref_out_opt LPDEVMODE            *ppCurrentDevmode
    )
...

{
    ...
    *pbComplete = FALSE;
    ...
    if (!GetStartPageListForwardForDriver(hSpoolHandle,
                                          pEMFAttr,
                                          pDevmode,
...

{
    ...
    *ppPageListHead = NULL;
    ...
    //
    if (!(pMemoryHead = (PPAGE_NUMBER)AllocSplMem( dwMemoryForPageList ))) {
...
BReverseSidesOnSheet(
        __in     PEMF_ATTRIBUTE_INFO    pEMFAttr,
        __inout  PPAGE_NUMBER           pPageList)
{
...
CopyDevmode(
    __in        PPRINTPROCESSORDATA pData,
    __deref_out LPDEVMODEW         *pDevmode)
...

{
    ...
    if (pData->pDevmode) {
        ...
        if (!(hWinSpoolDrv = HLoadLibraryFromSystemDirectory(TEXT("winspool.drv"))))
...

{
    ...
    if (!BRetrieveJobAttributes(&EMFAttr,
                                pData->pPrinterName,
                                pDevmode,
                                &AttributeInfo)) 
    {
        ODS(("BRetrieveJobAttributes failed\nPrinter %ws\nDocument %ws\nJobID %u\n", pData->pDevmode->dmDeviceName, pData->pDocument, pData->JobId));
        goto CleanUp;
    } 
    else 
    {
...
CleanUp:
    ...
    if ( EMFAttr.hSpoolss )
    {
        FreeLibrary (EMFAttr.hSpoolss);
        EMFAttr.hSpoolss              = NULL;
        EMFAttr.pfnGetJobAttributesEx = NULL;
    }
...
BGetPageCoordinatesForNUpFromArray(
                __in   PEMF_ATTRIBUTE_INFO   pEMFAttr,
                __out  PUpdateRect *         ppURect,
                __in   BOOL                  bLandscape)
{
    ENupDirection eNupDirection   = pEMFAttr->eNupDirection;
    DWORD dwNupDirection         = (DWORD)eNupDirection;
    DWORD dwNumberOfPagesPerSide = pEMFAttr->dwNumberOfPagesPerSide;
    ...
    )
{
    ...
    //First call level 4. If that fails, then call level 3 and use reasonable defaults
    //some defaults may be based on old behavior and assumptions.
    


    if (LoadLibGetJobAttributesEx(pEMFAttr,
                                  pPrinterName,
                                  pDevmode,
                                  pAttributeInfo
                                  ) )
    {
        // Things are good. Nothing more to be done.
    }


    else
    {
        if ( GetJobAttributes((LPWSTR)pPrinterName,
                              pDevmode,
                              (PATTRIBUTE_INFO_3)pAttributeInfo)
           )
        {
            // _ATTRIBUTE_INFO_4 specific settings
            ...
            // Use defaults based on old behavior
            pAttributeInfo->dwBookletFlags      = BOOKLET_EDGE_LEFT; // This flag is only looked if the wDrvJobOrderFlags/dwJobOrderFlags indicate BOOKLET_PRINT. 
        }

        else
        {
            bRetVal = FALSE;
            ODS(("Couldn't retrieve JobAttributes from the driver."));
        }
    ...
    for ( ; dwSides && NULL != pHead ; dwSides--, pHead = pHead->pNextSide )
    {
        ...
        for (pHeadLogical = pHead;NULL != pHeadLogical; pHeadLogical = pHeadLogical->pNextLogPage) 
        {
            ...
            if ( hEMF )
            {
                ...
                {
                    if ( !BPlayEmptyPages(hSpoolHandle, pEMFAttr, dwEmptyPagesToPrintBeforeNextValidPage, dwOptimization) )
                    {
        ...
        if ( 0 != dwEmptyPagesToPrintBeforeNextValidPage )
        {
            ...
            {
                BPlayEmptyPages(hSpoolHandle, pEMFAttr, dwEmptyPagesToPrintBeforeNextValidPage, dwOptimization);
                dwEmptyPagesToPrintBeforeNextValidPage = 0;
            }
        }
