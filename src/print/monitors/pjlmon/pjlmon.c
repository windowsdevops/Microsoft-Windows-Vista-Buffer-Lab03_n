...
DWORD   ProcessPJLString(__in PINIPORT, __in LPSTR, __out DWORD *);
VOID    ProcessParserError(DWORD);
VOID    InterpreteTokens(__in PINIPORT pIniPort, __in PTOKENPAIR tokenPairs, DWORD nTokenParsed);
BOOL    IsPJL(__in PINIPORT pIniPort);
BOOL    WriteCommand(__in HANDLE hPort, __in LPSTR cmd);
BOOL    ReadCommand(__in HANDLE hPort);
...
PJLMonOpenPortEx(
    ...
    __in    LPTSTR       pszPrinterName,
    __out   LPHANDLE     pHandle,
...
--*/
{
    ...
    if ( pIniPort->pIniJob &&
        ...
        (VOID) StringCchPrintfA (string, COUNTOF (string), "@PJL JOB NAME = \"MSJOB %d\"\015\012",
...
PJLMonGetPrinterDataFromPort(
    __in                        HANDLE  hPort,
                                DWORD   ControlID,
    __in_opt                    LPTSTR  pValueName,
    __in_bcount(cbInBuffer)     LPTSTR  lpInBuffer,
                                DWORD   cbInBuffer,
    __out_bcount(cbOutBuffer)   LPTSTR  lpOutBuffer,
                                DWORD   cbOutBuffer,
...
ProcessPJLString(
    __in    PINIPORT    pIniPort,
    __in    LPSTR       pInString,
    __out   DWORD      *lpcbProcessed
)
/*++
