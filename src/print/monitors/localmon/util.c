...
)
{
    if (LcmSpoolerSection.OwningThread != (HANDLE) UIntToPtr(GetCurrentThreadId())) {
        DBG_MSG(DBG_ERROR, ("Not in spooler semaphore\n"));
    }
}
...
{
    if (LcmSpoolerSection.OwningThread == (HANDLE) UIntToPtr(GetCurrentThreadId())) {
        DBG_MSG(DBG_ERROR, ("Inside spooler semaphore !!\n"));
    }
}
...
--*/
{
    ...
    {
        dwStatus = (pszCommValues = (PWSTR)AllocSplMem (cbData + sizeof(WCHAR))) ? ERROR_SUCCESS : ERROR_OUTOFMEMORY;
    }
...
--*/
{
    ...
    if (dwStatus == ERROR_SUCCESS)
    {
        dwStatus = (pszTimeout = (PWSTR)AllocSplMem (cbData + sizeof(WCHAR))) ? ERROR_SUCCESS : ERROR_OUTOFMEMORY;
    }
    //
    // get data
    //
