    ...
    )
{
    ...
    if ( IS_FILE_PORT(pName) ) {
        ...
        // to multiple files.
        //
        DBG_MSG(DBG_TRACE, ("Creating a new pIniPort for %ws\n", pName));
    ...
    __in    LPBYTE  pDocInfo)
{
    ...
    DWORD  Error = 0;
    LPWSTR pAdjustedName;

    DBG_MSG(DBG_TRACE, ("LcmStartDocPort(%08x, %ws, %d, %d, %08x)\n",
            ...
            __out               LPDWORD pcbWritten)
{
    ...
    DBG_MSG(DBG_TRACE, ("LcmWritePort(%08x, %08x, %d)\n", hPort, pBuffer, cbBuf));
    ...
    DBG_MSG(DBG_TRACE, ("LcmWritePort returns %d; %d bytes written\n", rc, *pcbWritten));
    ...
    __out               LPDWORD     pcbRead)
{
    ...
    DBG_MSG(DBG_TRACE, ("LcmReadPort(%08x, %08x, %d)\n", hPort, pBuffer, cbBuf));
    ...
    if (IS_COM_PORT(pIniPort->pName))
    {
        if (GetCOMPort(pIniPort))
        {
            ...
            DBG_MSG(DBG_TRACE, ("ReadPort returns %d; %d bytes read\n", rc, *pcbRead));
    ...
    )
{
    ...
    DBG_MSG(DBG_TRACE, ("LcmEndDocPort(%08x)\n", hPort));
    ...
    )
{
    ...
    DBG_MSG(DBG_TRACE, ("LcmClosePort\n"));
