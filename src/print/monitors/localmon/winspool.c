...
--*/
{
    ...
    if ( !QueryDosDevice(DosDeviceName, DeviceNames, COUNTOF (DeviceNames)) ) {
        ...
        bRet = TRUE;
        DBG_MSG(DBG_TRACE, ("ValidateDosDevicePort(): Unable to find port: %ws\n", DosDeviceName));
        goto Done;
    }
    ...
    if ( pIniPort->hFile == INVALID_HANDLE_VALUE ) {
        ...
        DBG_MSG(DBG_ERROR, 
                ("ValidateDosDevicePort(): CreateFile() failed!!!  GLE[%d]  portname: %ws\n", 
                GetLastError(),
                TempDosDeviceName));
    ...
    )
{
    ...
    if (frc && (pIniPort->hFile != INVALID_HANDLE_VALUE))
    {
        pIniPort->cRef++;
        DBG_MSG(DBG_TRACE,
                ("GetCOMPort(): increment cRef[%d] with FileHandle[%x]\n", pIniPort->cRef, pIniPort->hFile));
    }
    else
    {
        ...
        if (pIniPort->hFile == INVALID_HANDLE_VALUE)
        {
            ...
            DBG_MSG(DBG_ERROR, ("GetCOMPort(): Unable to get file handle!!!\n"));
        }
        else
        {
            DBG_MSG(DBG_ERROR, ("GetCOMPort(): ValidateDosDevicePort() failed!!!\n"));
        }
    }
    ...
    )
{
    ...
    DBG_MSG(DBG_TRACE,
            ("ReleaseCOMPort(): cRef[%d] using FileHandle[%x]\n", pIniPort->cRef, pIniPort->hFile) );
    ...
    DBG_MSG(DBG_TRACE,
            ("ReleaseCOMPort(): closed FileHandle[%x]\n", pIniPort->hFile) );
