...
{
    return(OutputBytes(pPDev, (LPBYTE)pszStr, strlen(pszStr)));
}
...
LONGToASCII(
                                           LONG    Number,
    __out_bcount_part(cchStr16, return+1)  LPSTR   pStr16,
                                           size_t  cchStr16,
                                           BYTE    NumType
    )
...
OutputXYParams(
                         PPDEV   pPDev,
    __in_ecount(cPoints) PPOINTL pPtXY,
    __in_opt             PPOINTL pPtOffset,
    __in_opt             PPOINTL pPtCurPos,
                         UINT    cPoints,
                         UINT    MaxCurPosSkips,
                         BYTE    NumType
    )
