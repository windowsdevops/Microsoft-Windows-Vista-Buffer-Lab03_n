...
Serenum_GetDevOtherID(
    __in_ecount(len)                PCHAR input,
                                    int   len,
    __out_ecount(MAX_DEVNODE_NAME)  LPSTR output
    );
...
void Serenum_GetDevName(
    __in_ecount(len)          PCHAR input,
                              int   len,
    __out_ecount(MAX_DEVNAME) PCHAR output,
    __inout                   int  *start
    );
...
Serenum_ParseData(        PFDO_DEVICE_DATA FdoData, 
   __in_ecount(BufferLen) PCHAR ReadBuffer, ULONG BufferLen,
...
Serenum_ReadSerialPort(
  __out_ecount(Buflen) OUT PCHAR PReadBuffer, IN USHORT Buflen,
...
SerenumScanOtherIdForMouse( 
    ...
    __in ULONG                          BufLen,
    __deref_out_ecount(*PmouseIdLen)    PCHAR *PpMouseId,
...
SerenumDoEnumProtocol(
    ...
    __deref_out_ecount(*PNBytes)  PUCHAR *PpBuf, 
                                  PUSHORT PNBytes,
...
SerenumCheckForLegacyDevice(IN PFDO_DEVICE_DATA PFdoData, 
     __in_ecount(BufferLen) IN PCHAR PReadBuf,
                            IN ULONG BufferLen,
