...
Arguments:
    ...
    argc - Number of command-line arguments passed to the executable
    				(including the executable name).
    
    argv - Pointer to the actual command-line parameters.
...
{  
	...
	//
	if (!MCAParseArguments(argc, argv)) {
...
MCAParseArguments(
	IN INT argc,
	__in_ecount(argc) IN PWCHAR argv[]
	)
/*++
 ...
 --*/
{
	...
	//
	if (_wcsicmp(argv[1], L"/corrected") == 0) {		
	
		gTimeOut = TIME_OUT_MAX;
 ...
 --*/
{
    ...
    wprintf(L"MCAMGMT /corrected 50\n\n");
