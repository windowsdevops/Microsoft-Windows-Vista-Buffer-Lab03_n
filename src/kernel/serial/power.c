...

{
   ...
   case IRP_MN_WAIT_WAKE:
      ...
      IoSetCompletionRoutine(PIrp, SerialWaitWakeCompletion, NULL,
                             TRUE, TRUE, TRUE);

      pDevExt->PendingWakeIrp = PIrp;
      
      PoCallDriver(pLowerDevObj, PIrp);
...

{
   ...
   IoCopyCurrentIrpStackLocationToNext(PIrp);
   IoSetCompletionRoutine(PIrp, SerialSetPowerD0Completion, NULL,
                          TRUE, TRUE, TRUE);
...
--*/
{
   ...
   status = PoRequestPowerIrp(PDevExt->Pdo, IRP_MN_WAIT_WAKE,
                              powerState, SerialWakeCompletion, PDevExt, NULL);
