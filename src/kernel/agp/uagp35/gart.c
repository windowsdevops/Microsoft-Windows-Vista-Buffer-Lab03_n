...
//            break;
    ...
    //
    {
        ...
        ApBaseMask32 = AP_BASE_MASK(ApSize);
        ApBaseMask = (LONGLONG)ApBaseMask32;
...
--*/
{
    ...
    // We have modified the gart table which is mapped as write combined so we flush the CPU WC buffer
    //
    NewPte.AsULONG = *(volatile ULONG *)AgpContext->Gart;   
...
{    
    ...
    AgpLibGetExtendedTargetCapability(AgpContext, AgpControl, &AgpDriverInfo->AGPReg1);
    AgpLibGetExtendedTargetCapability(AgpContext, AperturePageSize, &AgpDriverInfo->AGPReg2);
