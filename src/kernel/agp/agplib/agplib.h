...
#define AGP_BUGCHECK_INTERFACE_NOT_RELEASE  0x1
...
typedef struct _TARGET_EXTENSION {
    ...
    //
    // Location of the guard page that is mapped into unused Gart translation table entries
    //
    PHYSICAL_ADDRESS            AgpGuardPagePhys;
    __deref_volatile PVOID                       AgpGuardPageVirtual;
...
typedef struct _MASTER_EXTENSION {
    ...
    ULONG               Capabilities;
    __volatile ULONG               InterfaceCount;         // tracks the number of interfaces handed out
...
DRIVER_DISPATCH AgpDispatchPnp;
DRIVER_DISPATCH AgpDispatchPower;
DRIVER_DISPATCH AgpDispatchDeviceControl;
DRIVER_DISPATCH AgpDispatchWmi;
IO_COMPLETION_ROUTINE AgpSetEventCompletion;
FAST_IO_DETACH_DEVICE AGPFastIoDetachCallback;
...
IO_COMPLETION_ROUTINE AgpQueryResources;
...
AgpInterfaceMapMemory(
    ...
    IN ULONG OffsetInPages,
    IN PMDL Mdl,
    OUT PPHYSICAL_ADDRESS MemoryBase
    );
