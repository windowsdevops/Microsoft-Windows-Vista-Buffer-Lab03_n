...
--*/
{
    ...
    for (count = 0; (count < 4) && (count < StringLength); count++) {
    ...
    )
{
    ...
    LogPacket = IoAllocateErrorLogEntry(Extension->Target->Self,
    ...
    if (NULL != LogPacket) 
    {
        ...
        LogPacket->ErrorCode = ErrorCode;
        LogPacket->FinalStatus = FinalStatus;
