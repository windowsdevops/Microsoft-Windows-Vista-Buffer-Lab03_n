...
--*/
{
    ...
    AGPLOG(AGP_NOISE,
           ("Agp3QueryResourceRequirementsBridge: IRP %p, resource %08lx\n",
            Irp,
    ...
    if (!NT_SUCCESS(Status)) {
        AGPLOG(AGP_CRITICAL,
               ("Agp3QueryResourceRequirementsBridge: AgpQueryAperture %p "
                "failed %08lx\n",
                Extension,
                Status));
        return(Status);
    ...
    if (!NT_SUCCESS(RtlULongMult(ApertureRequirements->Count, OldRequirements->AlternativeLists, &SizeToAlloc)) ||
        !NT_SUCCESS(RtlULongMult(sizeof(IO_RESOURCE_DESCRIPTOR), SizeToAlloc, &SizeToAlloc)) ||
        !NT_SUCCESS(RtlULongAdd(NewSize, SizeToAlloc, &NewSize)))
    {
...

{
    ...
    AGPLOG(AGP_NOISE,
           ("AgpQueryResourceRequirements - IRP %p, resource %08lx\n",
            Irp,
            Irp->IoStatus.Information));
    ...
    if (!NT_SUCCESS(Status)) {
        AGPLOG(AGP_CRITICAL,
               ("AgpQueryResourceRequirements - AgpQueryAperture %p failed %08lx\n",
                Extension,
                Status));
        return(Status);
    }
    ...
    AGPLOG(AGP_NOISE,
           ("AgpQueryResourceRequirements - aperture at %I64x, length %08lx pages, Requirements %p\n",
            CurrentBase.QuadPart,
            CurrentSizeInPages,
            ApertureRequirements));
    ...
    if (!NT_SUCCESS(RtlULongMult(AddCount, OldRequirements->AlternativeLists, &SizeToAlloc)) ||
        !NT_SUCCESS(RtlULongMult(sizeof(IO_RESOURCE_DESCRIPTOR), SizeToAlloc, &SizeToAlloc)) ||
        !NT_SUCCESS(RtlULongAdd(NewSize, SizeToAlloc, &NewSize)))
    {
        if (ApertureRequirements) 
...

{
    ...
    AGPLOG(AGP_NOISE,
           ("AgpQueryResources - IRP %p, resource %08lx\n",
            Irp,
            Irp->IoStatus.Information));
    return(STATUS_SUCCESS);
}
...

{
    ...
    AGPLOG(AGP_NOISE,
           ("AgpStartTargetHost: IRP %p, resource %08lx\n",
            Irp,
            Irp->IoStatus.Information));
...

{
    ...
    AGPLOG(AGP_NOISE,
           ("Agp3StartTargetBridge: IRP %p, resource %08lx\n",
            Irp,
            Irp->IoStatus.Information));
