...

{
    ...
    if (!NT_SUCCESS(Status)) {
        AGPLOG(AGP_CRITICAL,
               ("AgpLibGetAgpCapability - read PCI Config space for Context %p failed %08lx\n",
                Context,
                Status));
        return(Status);
    }
    ...
    if ((PciConfig->Status & PCI_STATUS_CAPABILITIES_LIST) == 0) {
        AGPLOG(AGP_CRITICAL,
               ("AgpLibGetAgpCapability - Context %p does not support Capabilities list, not an AGP device\n",
                Context));
        return(STATUS_NOT_IMPLEMENTED);
    }
    ...
    while (CapabilityOffset != 0) {
        ...
        if (!NT_SUCCESS(Status)) {
            AGPLOG(AGP_CRITICAL,
                   ("AgpLibGetAgpCapability - read PCI Capability at offset %x for Context %p failed %08lx\n",
                    CapabilityOffset,
                    Context,
                    Status));
            return(Status);
    ...
    if (CapabilityOffset == 0) {
        ...
        //
        AGPLOG(AGP_CRITICAL,
               ("AgpLibGetAgpCapability - Context %p does have an AGP Capability entry, not an AGP device\n",
                Context));
        return(STATUS_NOT_IMPLEMENTED);
    }

    AGPLOG(AGP_CAP,
           ("AgpLibGetAgpCapability - Context %p has AGP Capability at offset %x\n",
            Context,
            CapabilityOffset));
...

{
    ...
    if (!NT_SUCCESS(Status)) {
        AGPLOG(AGP_CRITICAL,
               ("AgpLibGetAgpCapability - read AGP Capability at offset %x for Context %p failed %08lx\n",
                CapabilityOffset,
                Context,
                Status));
        return(Status);
    ...
    if (!NT_SUCCESS(Status)) {
        AGPLOG(AGP_CRITICAL,
               ("AgpLibGetExtendedAgpCapability - read extended AGP "
                "Capability at offset %x for Context %p failed %08lx\n",
                CapabilityOffset,
                Context,
                Status));
        return Status;
...

{
    ...
    if (!NT_SUCCESS(Status)) {
        AGPLOG(AGP_CRITICAL,
               ("AgpLibSetAgpCapability - Set AGP command at offset %x for Context %p failed %08lx\n",
                CapabilityOffset,
                Context,
                Status));
        return(Status);
    ...
    if (!NT_SUCCESS(Status)) {
        AGPLOG(AGP_CRITICAL,
               ("AgpLibSetExtendedAgpCapability - Set extended capability "
                "at offset %x for Context %p failed %08lx\n",
                CapabilityOffset,
                Context,
                Status));
    }
...
--*/
{
    ...
    if (Master) {
        ...
        if (!NT_SUCCESS(Status)) {
            AGPLOG(AGP_CRITICAL,
                   ("AgpLibGetMasterDeviceId - read PCI Config space for Context %p failed %08lx\n",
                    Master,
                    Status));
            return Status;
        }
