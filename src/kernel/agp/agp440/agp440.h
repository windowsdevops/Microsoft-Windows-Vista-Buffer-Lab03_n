...
#define GART_ENTRY_FREE         9           //  1001

#define GART_ENTRY_WC           2           //  0010
#define GART_ENTRY_UC           4           //  0100
...
#define GART_ENTRY_GUARD  (GART_ENTRY_WC | GART_ENTRY_UC | GART_ENTRY_VALID)

#define VERIFIER_ENABLED (AgpV_PTE.Hard.Valid)

#define VERIFIER_PFN(GartPte) \
    (((GartPte)->Hard.Page >= AgpV_PTE.Hard.Page) && \
    ((GartPte)->Hard.Page < (AgpV_PTE.Hard.Page + AGP_GUARD_RANGE_SIZE_IN_PAGES)))
...
typedef struct _AGP440_EXTENSION {
    ...
    __deref_volatile __field_ecount(GartLength) PGART_PTE Gart;
    ULONG               GartLength;
    PHYSICAL_ADDRESS    GartPhysical;
    ULONGLONG           SpecialTarget;
    ULONG               BootAGPCtlValue;
...
ULONG __inline ConvertPhysAddrToPTEVal(PHYSICAL_ADDRESS PhysAddr)
{
    ...
    AgpPTE32.AsUlong = 0;
    AgpPTE32.Hard.Page = (ULONG)((PhysAddr.QuadPart >> PAGE_SHIFT) & PAGE_LOW_MASK);
