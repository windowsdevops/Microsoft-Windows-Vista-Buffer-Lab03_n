...

{
    ...
    AgpV_PTE.AsUlong = ConvertPhysAddrToPTEVal(GuardPage);    
    AgpV_PTE.Hard.Valid = 1;    
...

{
    ...
    //
    // Store the boot time value of AGP control so we do not need to read-modify-write when
    // flushing the gart TLB
    //
    Extension->BootAGPCtlValue = AgpCtrl;
