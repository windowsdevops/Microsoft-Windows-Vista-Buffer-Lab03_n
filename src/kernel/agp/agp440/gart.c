...

{
    ...
    NewPte.AsUlong = AgpV_PTE.AsUlong;
#if (AGP_GUARD_RANGE_SIZE_IN_PAGES > 1) || AGP_GUARD_RANGE_UNMAP_HISTORY_SIZE
    NewPte.AsUlong = ConvertPhysAddrToPTEVal(AgpLibGuardRegionGetGuardPageAddr(AgpContext, ((PGART_PTE)Range->Context - &AgpContext->Gart[0]), Range->NumberOfPages, GART_OP_RELEASE));
#endif            
...

{
    ...
    NewPte.AsUlong = AgpV_PTE.AsUlong;
#if (AGP_GUARD_RANGE_SIZE_IN_PAGES > 1) || AGP_GUARD_RANGE_UNMAP_HISTORY_SIZE
    NewPte.AsUlong = ConvertPhysAddrToPTEVal(AgpLibGuardRegionGetGuardPageAddr(AgpContext, 0, GartLength / sizeof(GART_PTE), GART_OP_CREATE));
#endif        
    NewPte.Soft.State = GART_ENTRY_FREE;
    ...
    // We have modified the gart table which is mapped as write combined so we flush the CPU WC buffer
    //
    i = *(volatile ULONG *)AgpContext->Gart;
...

{
    ULONG PageCount;
    PGART_PTE Pte;
    __deref_volatile PGART_PTE StartPte;
    ULONG Index;
    ULONG TargetState;
    PPFN_NUMBER Page;
    BOOLEAN Backwards;
...
{    
    ...
    //
    // We flush the Gart TLB by disabling the use of the TLB and then enable it again
    // During this operation Gart translation is still enabled we just toggle the use
    // of the Gart TLB to speed up translations
    //
    AgpCtrl.AsUlong = AgpContext->BootAGPCtlValue;
...
Routine Description:
    ...
    This routine flushes the chipset posted write buffer to ensure that
    all pending read/write to memory are flushed thus ensuring any
    access through the AGP aperture will return the latest data written
    to by the CPU.
...
{    
    ...
    AgpLibReadAgpTargetConfig(AgpContext, &AgpDriverInfo->AGPReg1, AGPCTRL_OFFSET, sizeof(AgpDriverInfo->AGPReg1));
    AgpLibReadAgpTargetConfig(AgpContext, &AgpDriverInfo->AGPReg2, PACCFG_OFFSET, sizeof(AgpDriverInfo->AGPReg2));
