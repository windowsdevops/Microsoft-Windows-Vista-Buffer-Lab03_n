...
Routine Description:
    ...
    Invalidate Gart Translation Table TLB's for all KX CPU
...
--*/
{
    ...
    for (Index = 0; Index < KxAgpCtx.KxCount; Index++) {
        ...
        if ((PTE_Err & KX_GART_PTE_ERROR) == KX_GART_PTE_ERROR) {
            ...
            AGPLOG(AGP_CRITICAL, ("KxInvalidateGartTLB: PTE Error bit set!\n"));
            ...
            if (Length != sizeof(PTE_Err)) {
                AGPLOG(AGP_CRITICAL,
                       ("KxInvalidateGartTLB: Couldn't clear PTE Error bit!\n"));
            }
        }
#endif
...
VOID KxGetDriverAgpInfo(OUT PAGP_INFO_DRIVER AgpDriverInfo)
{
    ULONG   Length;

    Length = HalGetBusDataByOffset(PCIConfiguration,
                                       0,
                                       KxAgpCtx.KxSlot[0].u.AsULONG,
                                       &AgpDriverInfo->AGPReg1,
                                       KX_AP_CONTROL_OFFSET,
                                       sizeof(AgpDriverInfo->AGPReg1));    

    Length = HalGetBusDataByOffset(PCIConfiguration,
                                       0,
                                       KxAgpCtx.KxSlot[0].u.AsULONG,
                                       &AgpDriverInfo->AGPReg2,
                                       KX_GART_INV_OFFSET,
                                       sizeof(AgpDriverInfo->AGPReg2));      
}
