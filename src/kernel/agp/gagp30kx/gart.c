...
#pragma alloc_text(PAGE, AgpFlushChipsetCaches)
#endif
...
--*/
{
    ...
    // application looks there, and believes it has meaning...
    //
...
--*/
{
    ...
    AGP_ASSERT((Range->Type == MmNonCached) || 
    ...
    NewPte.AsULONG = ConvertPhysAddrToPTEVal(AgpLibGuardRegionGetGuardPageAddr(AgpContext, (FoundRange - &AgpContext->Gart[0]), Range->NumberOfPages, GART_OP_RESERVE));
#endif    
    ...
    //
    // We have modified the gart table which is mapped as write combined so we flush the CPU WC buffer
    //
    Index = *(volatile ULONG *)AgpContext->Gart;    
...
--*/
{
    ...
    NewPte.AsULONG = AgpV_PTE32.AsULONG;
#if (AGP_GUARD_RANGE_SIZE_IN_PAGES > 1) || AGP_GUARD_RANGE_UNMAP_HISTORY_SIZE
    NewPte.AsULONG = ConvertPhysAddrToPTEVal(AgpLibGuardRegionGetGuardPageAddr(AgpContext, ((PGART_PTE32)Range->Context - &AgpContext->Gart[0]), Range->NumberOfPages, GART_OP_RELEASE));
#endif        
    ...
    // We have modified the gart table which is mapped as write combined so we flush the CPU WC buffer
    //
    Start = *(volatile ULONG *)AgpContext->Gart;   
...
--*/
{
    ...
    // entire aperture
    //
    LowestAcceptable.QuadPart = 0x100000;
    ...
    __assume_bound(AgpContext->ApertureLength);
    GartLength =
    ...
    NewPte.AsULONG = AgpV_PTE32.AsULONG;
#if (AGP_GUARD_RANGE_SIZE_IN_PAGES > 1) || AGP_GUARD_RANGE_UNMAP_HISTORY_SIZE
    NewPte.AsULONG = ConvertPhysAddrToPTEVal(AgpLibGuardRegionGetGuardPageAddr(AgpContext, 0, GartLength / sizeof(GART_PTE32), GART_OP_CREATE));
#endif        
    NewPte.Soft.State = GART_ENTRY_FREE;
    ...
    if (AgpV_Flags & AGP_GART_GUARD_VERIFICATION) {
        ...
        NewPte.Soft.State = GART_ENTRY_GUARD;
        for (Offset = 0; Offset < Ap4thPages; Offset++) {
            Gart[Offset] = NewPte;
            Gart[Index - (1 + Offset)] = NewPte;
        }
    }
...
--*/
{
    ...
    ULONG PageCount;
    PGART_PTE32 Pte;
    __deref_volatile PGART_PTE32 StartPte;
    ULONG Index;
    ULONG TargetState;
    PPFN_NUMBER Page;
    BOOLEAN Backwards;
    ...
    AgpLibGuardRegionGetGuardPageAddr(AgpContext, (Pte - &AgpContext->Gart[0]), PageCount, GART_OP_MAP);
#endif            

    for (Index = 0; Index < PageCount; Index++, Pte++) 
    {
        ASSERT(GartEntryReservedAs(Pte, TargetState));

        NewPte.Hard.PageLow = (ULONG)(*Page & PAGE_LOW_MASK);
        NewPte.Hard.PageHigh = (ULONG)((*Page++ >> PAGE_HIGH_SHIFT) & PAGE_HIGH_MASK);

        Pte->AsULONG = NewPte.AsULONG;
        AGP_ASSERT(Pte->Hard.Valid == ON);
    }
    ...
    // We have modified the gart table which is mapped as write combined so we flush the CPU WC buffer
    //
    NewPte.AsULONG = *(volatile ULONG *)StartPte;
...
--*/
{
    ...
    if ((OffsetInPages > AgpRange->NumberOfPages) ||
        (NumberOfPages > AgpRange->NumberOfPages) ||
        ((OffsetInPages + NumberOfPages) > AgpRange->NumberOfPages)) 
    {
        return STATUS_UNSUCCESSFUL;
    }
    ...
    NewPte.AsULONG = AgpV_PTE32.AsULONG;
#if (AGP_GUARD_RANGE_SIZE_IN_PAGES > 1) || AGP_GUARD_RANGE_UNMAP_HISTORY_SIZE
    NewPte.AsULONG = ConvertPhysAddrToPTEVal(AgpLibGuardRegionGetGuardPageAddr(AgpContext, (&StartPte[OffsetInPages] - &AgpContext->Gart[0]), NumberOfPages, GART_OP_UNMAP));
#endif            
    NewPte.Soft.State = NewState;
    ...
    if (AgpV_Flags & AGP_GART_CORRUPTION_VERIFICATION) 
    {
        ...
        {
#pragma prefast(suppress:__WARNING_REDUNDANTTEST, "by design; different macros w/ same value")
    ...
    for (Index = 0; Index < NumberOfPages; Index++, Pte++) 
    {        
        ASSERT(GartEntryMapped(Pte));                   
        *Pte = NewPte;
    }
...
Routine Description:
    ...
    This routine flushes all pending write to memory coming from the CPU or GPU
    to system memory or to the AGP aperture range to ensure coherent view of
    memory.  
...
--*/
{
    ...
    //
    // Flushing all pending CPU writes is different on AMD, VIA and SIS chipset.
    //
    // 1) AMD chipset: there is nothing to do.
    // 2) SiS chipset: we need to read a non-cacheable memory location.
    // 3) Via chipset: we need to write to a non-cacheable memory location than 
    //    read it back. 
    //
    // Since this is not a performance critical code path, we implement
    // the VIA workaround which is a superset of the other two.
    //

    if (NULL != AgpContext->Gart) 
    {
        ULONG Dummy;

        //
        // Read the first entry of the GART so we don't corrupt it.
        //

        Dummy = AgpContext->Gart[0].AsULONG;

        //
        // Write than read the first entry of the GART back to flush
        // the posted write buffer.
        //

        AgpContext->Gart[0].AsULONG = Dummy;
        Dummy = AgpContext->Gart[0].AsULONG;
    }
...
Arguments:
    ...
    AgpDriverInfo - Structure where to information will be stored

Return Value:

    None, rotuine should gather as much info as posible

--*/         
{    
    AgpDriverInfo->ApertureStart = AgpContext->ApertureStart;
    AgpDriverInfo->GartTranslationTable = AgpContext->GartPhysical;
    AgpDriverInfo->ApertureLength = AgpContext->ApertureLength;

    KxGetDriverAgpInfo(AgpDriverInfo);
